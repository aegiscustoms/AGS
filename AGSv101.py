...
# 화면 전체를 제어할 가림막 컨테이너 생성 (번쩍임 방지의 핵심)
main_placeholder = st.empty()

if not st.session_state.logged_in:
    # 로그인이 안 된 경우, 가림막 안에 로그인 폼만 표시
                ...
                if submit:
                    ...
                    with st.spinner("보안 인증 및 데이터 연결 중..."):
                        ...
                        if input_id == "aegis01210" and input_pw == "dlwltm2025@":
                            ...
                            main_placeholder.empty() # 로그인 성공 시 폼 제거
                            st.rerun()
                        ...
                        # 2) 일반 고객사 체크
                            ...
                            if 'ID' in df_check.columns:
                                ...
                                if not user_match.empty:
                                    ...
                                    st.error(f"❌ 등록되지 않은 아이디입니다.")
                            else:
                                st.error("⚠️ 데이터베이스 구조 오류")
                        else:
                            st.error("⚠️ 데이터베이스 연결 실패")
    # 로그인이 완료되기 전까지는 아래의 모든 코드(메인 화면)가 실행되지 않도록 차단
    st.stop()
