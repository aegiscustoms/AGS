...
# --- [47행 시작] 쿠키 매니저 및 세션 복구 (UI 안정화 버전) ---
cookie_manager = stx.CookieManager()

# 새로고침 시 쿠키가 브라우저에서 서버로 전달될 때까지 충분히 대기
if 'cookie_wait_done' not in st.session_state:
    time.sleep(0.8)  # 대기 시간을 0.8초로 약간 늘려 안정성을 확보합니다.
    st.session_state.cookie_wait_done = True

saved_user_id = cookie_manager.get(cookie="aegis_user_id")

# 필수 세션 변수 초기화 (AttributeError 방지 핵심)
...
# 쿠키를 통한 자동 로그인 복구 시도
if not st.session_state.logged_in and saved_user_id:
    try:
                    ...
                    with st.spinner("보안 인증 중..."):
                        try:
                            ...
                            if input_id == "aegis01210" and input_pw == "dlwltm2025@":
                                ...
                                cookie_manager.set("aegis_user_id", input_id, key="admin_cookie")
                                st.rerun()
                                ...
                                if 'ID' in df_check.columns:
                                    ...
                                    if not user_match.empty:
                                        ...
                                        if input_pw_hash == db_pw:
                                            st.session_state.logged_in = True
                                            st.session_state.user_id = input_id
                                            st.session_state.user_name = user_match.iloc[0].get('NAME', input_id)
                                            st.session_state.is_admin = False
                                            raw_tabs = user_match.iloc[0].get('ACCESS_TABS', "🔍 HS검색, 📘 HS정보, 📊 통계부호, 📦 화물통관진행정보, 🧮 세액계산기")
                                            st.session_state.user_permissions = [t.strip() for t in str(raw_tabs).split(',')]
                                            cookie_manager.set("aegis_user_id", input_id, key="user_cookie")
                                            st.rerun()
...
# --- 로그인 이후 공통 헤더 및 사이드바 ---
st.sidebar.markdown(f"### 👤 {st.session_state.get('user_id', '')} ({st.session_state.get('user_name', '')})")
if st.sidebar.button("로그아웃", key="logout_final"):
    st.session_state.clear() # 모든 세션 초기화
    cookie_manager.delete("aegis_user_id")
    st.rerun()

# 탭 생성 로직 (246행 부근 에러 발생 지점)
