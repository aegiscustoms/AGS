...
if st.session_state.logged_in:
    # 화면을 쪼개서 우측 끝에 버튼과 이름을 배치 (비율 조정: 7.8 : 1.0 : 1.2)
    col_space, col_logout, col_name = st.columns([7.8, 1.0, 1.2])
    ...
    with col_logout:
        ...
        if st.button("🔓 로그아웃", key="top_logout_btn"):
            st.session_state.logged_in = False
            try:
                # 1단계에서 선언한 매니저를 사용하여 이제 에러가 나지 않습니다.
    ...
    with col_name:
        # 2) 상호명 (우측 끝 배치)
        st.markdown(f"""
            <div style="background-color: #f8f9fa; padding: 7px 15px; border-radius: 20px; 
            border: 1px solid #e2e8f0; color: #1E3A8A; font-weight: 700; font-size: 14px; 
