        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("⏳ AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    9. (검증) 도출한 번호가 아래 제공된 [실제 HS Code 리스트]에 있는지 확인하세요. 만약 당신이 생각한 번호가 리스트에 없다면, 리스트 내에서 가장 유사하고 타당한 번호를 최종 '추천 HS Code'로 선정하되, 분류 근거에서 그 이유를 설명하세요.
                    """
                    ...
                    [참조: 실제 HS Code 리스트]
                    {actual_codes_pool if actual_codes_pool else "리스트를 불러올 수 없습니다. 대한민국 관세율표 규격(10단위)에 맞춰 분석하세요."}
