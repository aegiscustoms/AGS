...
import time
import os
from streamlit_gsheets import GSheetsConnection # ◆변경내역◆추가
...
# ◆변경내역◆시작◆ [구글 시트 및 로그인 인증 로직 수정본]
try:
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    df_users = conn_gs.read(worksheet="users", ttl=0)
except Exception as e:
    df_users = pd.DataFrame(columns=["id", "pw", "name", "is_approved", "is_admin", "access_tabs"])
...
if not st.session_state.logged_in:
    ...
    with cl2:
        with st.form("login_form"):
            l_id = st.text_input("아이디 (사업자번호)").strip().replace("-", "")
            l_pw = st.text_input("비밀번호", type="password").strip()
            if st.form_submit_button("로그인"):
                # 1) 관리자 하드코딩 체크
                if l_id == "aegis01210" and l_pw == "dlwltm2025@":
                ...
                # 2) 고객사 구글 시트 체크
                    ...
                    input_pw_hash = hashlib.sha256(l_pw.encode()).hexdigest().lower()
                    df_clean = df_users.copy().astype(str)
                    df_clean['id'] = df_clean['id'].str.replace("-", "").str.strip()
                    df_clean['pw'] = df_clean['pw'].str.strip().str.lower()
                    user_match = df_clean[(df_clean['id'] == l_id) & (df_clean['pw'] == input_pw_hash)]
                    if not user_match.empty:
                        ...
                        if user_info['is_approved'] == '1':
                            ...
                            st.session_state.access_tabs = user_info.get('access_tabs', 'ALL')
                            st.rerun()
                        else: st.error("승인 대기 중인 계정입니다.")
...
st.sidebar.markdown(f"### 👤 {st.session_state.user_id} ({st.session_state.user_name})")
if st.sidebar.button("로그아웃"): st.session_state.logged_in = False; st.rerun()

# ◆변경내역◆시작◆ [신규 기능: 화주 전용 회원계정관리 탭 추가 반영]
main_tabs = ["🔍 HS검색", "📘 HS정보", "📊 통계부호", "📦 화물통관진행정보", "🧮 세액계산기"]
if st.session_state.is_admin:
    tabs = st.tabs(main_tabs + ["⚙️ 관리자"])
else:
    tabs = st.tabs(main_tabs + ["👤 계정관리"])
# ◆변경내역◆끝◆
...
if not st.session_state.is_admin:
        ...
        with c_p1:
            ...
            with st.form("pw_change_form"):
                ...
                if st.form_submit_button("변경 사항 저장", use_container_width=True):
                    ...
                    # 현재 세션 유저의 데이터 추출 (사업자번호 하이픈 제거 대조)
                    user_row = df_users[df_users['id'].astype(str).str.replace("-","") == st.session_state.user_id]
                    
                    if not user_row.empty and curr_hash == user_row.iloc[0]['pw'].strip().lower():
                        if new_pw == new_pw_chk:
                            if len(new_pw) < 4:
                                st.error("❌ 새 비밀번호는 4자리 이상이어야 합니다.")
                            else:
                                new_hash = hashlib.sha256(new_pw.encode()).hexdigest().lower()
                                df_users.loc[df_users['id'].astype(str).str.replace("-","") == st.session_state.user_id, 'pw'] = new_hash
                                # 구글 시트 업데이트 실행
                                conn_gs.update(worksheet="users", data=df_users)
                                st.success("✅ 비밀번호가 안전하게 변경되었습니다.")
                        else: st.error("❌ 새 비밀번호가 일치하지 않습니다.")
        ...
        with c_p2:
            ...
            with st.form("inquiry_form"):
                ...
                if st.form_submit_button("문의 제출하기", use_container_width=True):
                    # 추후 문의 내용을 구글 시트에 기록하거나 메일로 발송하는 로직 확장 가능
                    st.success("✅ 문의가 정상적으로 접수되었습니다. 담당 관세사가 확인 후 연락드리겠습니다.")
    ...
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            # --- [서브탭 2: 고객사 계정 통합 제어 - ◆변경내역◆시작◆ (사업자번호/구글시트 연동)] ---
            with adm_tabs[1]:
                st.subheader("👤 고객사 계정 통합 제어 (사업자번호 ID)")
                
                with st.expander("➕ 신규 고객사 직접 등록"):
                    c1, c2, c3 = st.columns(3)
                    nid = c1.text_input("사업자번호 (ID)").replace("-", "")
                    nnm = c2.text_input("업체명")
                    npw = c3.text_input("초기 비밀번호", value="1234", type="password")
                    all_funcs = ["HS검색", "HS정보", "통계부호", "통관정보", "세액계산"]
                    sel_tabs = st.multiselect("접근 허용 기능", all_funcs, default=all_funcs)
                    
                    if st.button("고객사 등록 실행", use_container_width=True):
                        if nid and nnm:
                            hashed_pw = hashlib.sha256(npw.encode()).hexdigest().lower()
                            new_row = pd.DataFrame([{"id": nid, "pw": hashed_pw, "name": nnm, "is_approved": 1, "is_admin": 0, "access_tabs": ",".join(sel_tabs)}])
                            updated_df = pd.concat([df_users, new_row], ignore_index=True)
                            conn_gs.update(worksheet="users", data=updated_df)
                            st.success(f"✅ {nnm} ({nid}) 등록이 완료되었습니다.")
                            time.sleep(1); st.rerun()

                st.divider()
                with st.expander("📂 CSV 파일 일괄 등록"):
                    st.info("CSV 양식: id, name, pw (비밀번호는 원문으로 작성 시 자동 암호화)")
                    u_file = st.file_uploader("CSV 고객 명단 업로드", type="csv", key="bulk_csv_up")
                    if u_file and st.button("CSV 일괄 반영하기", use_container_width=True):
                        df_upload = pd.read_csv(u_file)
                        df_upload['pw'] = df_upload['pw'].astype(str).apply(lambda x: hashlib.sha256(x.encode()).hexdigest().lower())
                        df_upload['is_approved'] = 1
                        df_upload['is_admin'] = 0
                        df_upload['access_tabs'] = "ALL"
                        final_df = pd.concat([df_users, df_upload], ignore_index=True)
                        conn_gs.update(worksheet="users", data=final_df)
                        st.success("✅ 고객사 대량 등록이 완료되었습니다.")
                        time.sleep(1); st.rerun()

                st.divider()
                st.write("**📊 현재 등록 고객사 목록 (Google Sheets 연동)**")
                # 관리자 계정 제외한 리스트업
                display_df = df_users[df_users['is_admin'].astype(str) == '0']
                if not display_df.empty:
                    st.dataframe(display_df[['id', 'name', 'access_tabs']], use_container_width=True, hide_index=True)
                    target = st.selectbox("관리 및 제어 대상 선택", display_df['id'].tolist())
                    cr1, cr2 = st.columns(2)
                    if cr1.button("🔑 선택 업체 비밀번호 1234 초기화", use_container_width=True):
                        df_users.loc[df_users['id'].astype(str) == str(target), 'pw'] = hashlib.sha256("1234".encode()).hexdigest().lower()
                        conn_gs.update(worksheet="users", data=df_users)
                        st.success(f"✅ {target} 비밀번호 초기화 완료")
                    if cr2.button("🗑️ 해당 고객사 계정 삭제", type="primary", use_container_width=True):
                        df_users = df_users[df_users['id'].astype(str) != str(target)]
                        conn_gs.update(worksheet="users", data=df_users)
                        st.warning(f"⚠️ {target} 계정 삭제 완료")
                        time.sleep(1); st.rerun()
                else:
                    st.info("현재 등록된 고객사 계정이 없습니다.")
            # ◆변경내역◆끝◆

            # --- [서브탭 3: 이용 로그 확인 - 기존 서식 유지] ---
            with adm_tabs[2]:
                st.subheader("📋 시스템 이용 로그")
                log_data = [{"일시": "2026-03-19 15:30", "ID": "user1", "업체명": "테스트", "활동": "HS조회"}]
                st.table(log_data)
