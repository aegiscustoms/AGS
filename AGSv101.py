                    ...
                    with cols[i%3]:
                        ...
                        if up and st.button(f"반영", key=f"btn_{m_name}_f", use_container_width=True):
                            ...
                            if df_upload is not None:
                                ...
                                target_table = table_map.get(m_name)
                                if target_table:
                                    # 기존 데이터를 지우고 새로 덮어쓰기 (if_exists='replace')
                                    df_upload.to_sql(target_table, conn, if_exists='replace', index=False)
                                    conn.close()
                                    st.session_state.upload_status[m_name] = True
                                    st.success(f"✅ {m_name} 데이터가 DB에 성공적으로 반영되었습니다.")
                                    time.sleep(1)
                                    st.rerun()
                                else:
                                    st.error("테이블 매핑 정보를 찾을 수 없습니다.")
                            # [기존 업로드 로직 유지]
                            st.session_state.upload_status[m_name] = True
