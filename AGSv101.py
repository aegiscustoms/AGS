...
# --- 하단 푸터 (유저정보 및 Logout 통합형 - 높이 50px 교정) ---
st.divider()
...
with f_right:
    ...
    if st.session_state.get('logged_in', False):
        # 높이 50px에 맞춘 내부 컬럼 배치 (vertical_alignment로 버튼 높이 조절)
        u_col1, u_col2 = st.columns([0.6, 0.4], vertical_alignment="center")
        
        with u_col1:
            # 컨테이너 높이 50px, 텍스트 정중앙 배치
            st.markdown(f"""
        ...
        with u_col2:
            # Logout 버튼 (버튼도 함께 정렬되도록 마진 살짝 조정)
