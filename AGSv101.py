...
# ★변경내역★시작★ [1단계: 시스템 로딩 바 투명화]
st.markdown("""
    <style>
        /* 시스템 상태 표시기(녹색 글씨)를 아예 투명하게 만듦 */
...
# ★변경내역★시작★ [하단 플래시 물리적 차단 및 로그인 로직]
...
# 2. 로그인이 되지 않았을 때의 처리 (이 블록이 화면 전체를 제어함)
if not st.session_state.logged_in:
    with login_screen.container():
        ...
        with cl2:
            with st.form("login_form"):
                ...
                if submit:
                    with st.spinner("보안 인증 확인 중..."):
                        # 로그인 버튼 클릭 시에만 데이터를 가져와 로딩 메시지 노출 최소화
                        df_users = conn_gs.read(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", ttl=0)                        
                        ...
                        if input_id == "aegis01210" and input_pw == "dlwltm2025@":
                            ...
                            write_log(input_id, "마스터 관리자", "로그인", "관리자 접속")
                            st.rerun()
                            ...
                            if 'ID' in df_check.columns:
                                ...
                                if not user_match.empty:
                                    ...
                                    if input_pw_hash == db_pw:
                                        ...
                                        write_log(input_id, u_name, "로그인", "고객사 접속")
                                        st.rerun()
                                    else:
    ...
    # [방어막의 핵심] 로그인이 안 된 상태라면 여기서 무조건 실행을 멈춤. 
    # 이 아래의 탭 생성, 메인 로직 등은 아예 읽지도 않으므로 하단 플래시가 물리적으로 불가능함.
    st.stop() 
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                ...
                if not df_users.empty:
                    ...
                    # [1] 데이터 클리닝: ID의 '.0' 제거 및 문자열 표준화 (로그인 시 사용한 로직과 동일)
                    def clean_format(x):
                        s = str(x).strip()
                    ...
                    # 관리자 계정(aegis01210) 제외 필터링
                    display_df = display_df[display_df['ID'] != "aegis01210"]
                    
                    # [2] 목록 표시 (표 형식 - 5줄 높이 스크롤 유지)
                    st.dataframe(
