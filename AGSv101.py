...
# --- [기존 125행 ~ 160행 부근 대체] ---
# ◆변경내역◆시작◆ [구글 시트 및 로그인 인증 로직 최종 수정본]
try:
    conn_gs = st.connection("gsheets", type=GSheetsConnection)
    df_users = conn_gs.read(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", ttl=0)
except Exception as e:
    df_users = pd.DataFrame(columns=["ID", "Password", "Name", "BizNo", "Email", "Level", "JoinDate", "Status"])
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인"):
                ...
                if l_id == "aegis01210" and l_pw == "dlwltm2025@":
                    ...
                    st.rerun()
                # 2) 고객사 구글 시트 체크 (대문자 헤더 ID, Password 적용)
                else:
                    ...
                    if not df_users.empty and 'ID' in df_users.columns:
                        ...
                        # 시트의 대문자 'ID'와 'Password' 컬럼을 사용하도록 수정
                        df_clean['ID'] = df_clean['ID'].str.replace("-", "").str.strip()
                        df_clean['Password'] = df_clean['Password'].str.strip().str.lower()
                        
                        user_match = df_clean[(df_clean['ID'] == l_id) & (df_clean['Password'] == input_pw_hash)]
                        
                        if not user_match.empty:
                            user_info = user_match.iloc[0]
                            # Status가 '활성'인 경우에만 로그인 허용
                            if user_info.get('Status') == '활성':
                                st.session_state.logged_in = True
                                st.session_state.user_id = l_id
                                st.session_state.user_name = user_info.get('Name', l_id)
                        ...
                        st.error("⚠️ 사용자 데이터베이스 구조 오류 (ID 컬럼 없음)")
    st.stop()
# ◆변경내역◆끝◆
...
if not st.session_state.is_admin:
    ...
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                st.subheader("👥 고객사 계정 관리 (Google Sheets)")
                
                # [1] 신규 고객사 직접 등록 (이메일 선택 입력 반영)
                with st.expander("➕ 신규 고객사 직접 등록"):
                    ...
                    if st.button("고객사 등록 실행", use_container_width=True):
                        if nid and nnm:
                            ...
                            # 구글 시트 헤더(ID, Password, Name...)와 정확히 일치시킴
                ...
                # [3] 고객사 목록 조회 (5줄 높이 스크롤 적용)
                st.write("**📊 현재 등록 고객사 명단**")
                # 관리자 계정(aegis01210) 제외 필터링 (is_admin 대신 ID 컬럼 사용)
                display_df = df_users[df_users['ID'].astype(str) != "aegis01210"]
                
                if not display_df.empty:
                    # height=230 설정 시 약 5~6줄 정도 보이고 스크롤바가 생깁니다.
                    st.dataframe(
                        display_df[['ID', 'Name', 'Email', 'JoinDate', 'Status']], 
                    ...
                    # 계정 관리 제어부
                    target = st.selectbox("🛠️ 관리 대상 업체 선택", display_df['ID'].tolist())
                    cr1, cr2 = st.columns(2)
                    if cr1.button("🔑 비밀번호 1234로 초기화", use_container_width=True):
                        df_users.loc[df_users['ID'].astype(str) == str(target), 'Password'] = hashlib.sha256("1234".encode()).hexdigest().lower()
                        conn_gs.update(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", data=df_users)
                        st.success(f"✅ {target} 비밀번호가 초기화되었습니다.")
                    if cr2.button("🗑️ 계정 즉시 삭제", type="primary", use_container_width=True):
                        df_users = df_users[df_users['ID'].astype(str) != str(target)]
                        conn_gs.update(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", data=df_users)
                        st.warning(f"⚠️ {target} 계정이 삭제되었습니다.")
                        time.sleep(1); st.rerun()
                else:
