            ...
            if submit:
                ...
                # 2) 일반 고객사 체크
                    ...
                    if 'ID' in df_check.columns:
                        # [핵심] ID 비교를 위해 모든 데이터를 문자열로 바꾸고 소수점(.0) 및 공백 제거
                        def clean_id(x):
                            val = str(x).strip().replace("-", "")
                            if val.endswith('.0'): val = val[:-2] # 8764101210.0 방지
                            return val
                        ...
                        if not user_match.empty:
                            ...
                            st.error(f"❌ 등록되지 않은 아이디입니다. (입력: {input_id})")
                    else:
                        st.error("⚠️ 시트 헤더 오류: 'ID' 컬럼이 없습니다.")
                else:
                    st.error("⚠️ 데이터베이스를 불러올 수 없습니다.")
    st.stop()
# ◆변경내역◆끝◆
