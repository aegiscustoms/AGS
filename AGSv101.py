...
# 3. [최종 해결] 절대 위치(Absolute) 방식을 통한 겹침 및 여백 원천 차단
if st.session_state.logged_in:
    st.markdown(f"""
        <style>
            ...
            /* 2. 탭 메뉴 위치 조정 (상단으로 끌어올림) */
            .stTabs {{ margin-top: -30px; }}

            /* 3. 유저 정보 & 버튼 컨테이너 (우측 상단 절대 위치 고정) */
            .fixed-top-right {{
                position: absolute;
                top: -42px; /* 탭 메뉴와 수평을 맞추기 위한 높이 조정 */
                right: 0px;
                display: flex;
                align-items: center;
                gap: 8px;
            ...
            /* Logout 버튼용 투명 공간 확보 (실제 버튼은 아래에서 생성) */
            .btn-spacer {{ width: 70px; height: 30px; }}
        </style>
        
        <div style="position: relative;">
            <div class="fixed-top-right">
                <div class="user-tag">{st.session_state.user_name}님</div>
                <div class="btn-spacer"></div>
            </div>
        </div>
    """, unsafe_allow_html=True)

    # 실제 로그아웃 버튼 (HTML 레이아웃의 btn-spacer 위치에 정확히 겹치게 배치)
    # columns 비율을 조절하여 버튼을 오른쪽 끝으로 보냅니다.
    _, col_logout = st.columns([9.25, 0.75])
    with col_logout:
        # 탭 메뉴 라인과 높이를 맞추기 위한 마진 조정
        st.write("<div style='margin-top: -50px; position: relative; z-index: 1002;'>", unsafe_allow_html=True)
        if st.button("Logout", key="top_logout_btn"):
            st.session_state.logged_in = False
            try: cookie_manager.delete("aegis_user_id")
            except: pass
            st.rerun()
