...
# 3. [핵심 수정] 한 줄(Row)을 쪼개서 왼쪽엔 탭, 오른쪽엔 유저 정보 배치
# 비율 설명: 7.0(탭 메뉴) : 1.0(로그아웃 버튼) : 2.0(유저명)
col_tab, col_logout, col_name = st.columns([7.0, 1.0, 2.0])

with col_tab:
    # 왼쪽에 탭 생성 (메뉴 권한 체크 포함)
...
# 로그인이 된 상태일 때만 우측 정보 표시
if st.session_state.logged_in:
    with col_logout:
        # 오른쪽 정렬을 위해 마진을 살짝 준 로그아웃 버튼
    ...
    with col_name:
        # 가장 오른쪽 끝: 유저 상호명
        st.markdown(f"""
            <div style="background-color: #f8f9fa; padding: 6px 12px; border-radius: 15px; 
...
# 탭 메뉴와 본문 사이의 간격 조정 (너무 넓으면 삭제 가능)
