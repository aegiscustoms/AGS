...
# 2. [최종 해결] 상단바 통합 HTML 출력 (탭 메뉴와 같은 높이에 유저정보 배치)
if st.session_state.get('logged_in', False):
    # HTML로 유저 정보창과 버튼 자리를 미리 만듭니다.
    st.markdown(f"""
        <div class="aegis-top-right-zone">
            <div class="user-tag-min">{st.session_state.user_name}님</div>
            <div style="width: 65px; height: 28px;"></div> </div>
    """, unsafe_allow_html=True)

    # 실제 로그아웃 버튼 (HTML에서 예약한 65px 자리 위로 강제 이동)
    st.write('<div style="position: absolute; top: -43px; right: 0px; z-index: 1002;">', unsafe_allow_html=True)
