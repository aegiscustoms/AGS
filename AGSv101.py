...
st.markdown("""
    <style>
        ...
        /* 탭 메뉴가 있는 컨테이너의 상단 여백 조정 */
        .stTabs { margin-top: -45px; } 

        /* 유저 정보박스: 위치를 고정(fixed)하지 않고 탭 메뉴 우측에 플로팅 */
        .top-user-info-box {
            float: right;
            display: flex;
            align-items: center;
...
# 1. 전체 메뉴 구성 정의
# --- [메인 레이아웃: 탭 메뉴와 유저 정보(로그아웃/상호명) 한 줄 통합 배치] ---

# 1. 전체 메뉴 구성 정의
# --- [메인 레이아웃: 탭 메뉴와 유저 정보(상호명/로그아웃) 한 줄 통합 배치] ---
...
# 3. [최종 수정] 탭 메뉴 우측 상단에 유저 정보를 '끼워넣는' 레이아웃
# 유저 정보가 본문을 밀어내지 않도록 세로 높이를 0으로 제어하는 컨테이너를 먼저 배치합니다.
if st.session_state.logged_in:
    # 탭 메뉴 위쪽/오른쪽에 딱 붙이기 위한 플로팅 레이아웃
    st.markdown(f"""
        <div style="position: relative; width: 100%; height: 0px; z-index: 999;">
            <div style="position: absolute; right: 0; top: 5px; display: flex; align-items: center; gap: 8px;">
                <div style="background-color: #f8f9fa; padding: 5px 12px; border-radius: 15px; 
                            border: 1px solid #e2e8f0; color: #1E3A8A; font-weight: 700; 
                            font-size: 13px; white-space: nowrap; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
    ...
    # 로그아웃 버튼은 Streamlit 기능을 유지하기 위해 별도 컬럼 배치
    # 비율을 조절하여 버튼이 상호명 바로 오른쪽에 오도록 맞춤
    c_space, c_logout = st.columns([9.1, 0.9])
    with c_logout:
        # 버튼 상단 마진을 주어 상호명 박스와 높이를 맞춤
        st.write("<div style='margin-top: 2px;'>", unsafe_allow_html=True)
        if st.button("🔒 로그아웃", key="top_logout_btn"):
            st.session_state.logged_in = False
            try:
...
# 4. 탭 생성 (이제 유저 정보가 탭 우측 상단에 떠 있는 상태가 됩니다)
...
# 탭 메뉴 하단 구분선 및 여백 정리
st.markdown("<div style='margin-top: -15px; border-bottom: 1px solid #E2E8F0; margin-bottom: 15px;'></div>", unsafe_allow_html=True)
