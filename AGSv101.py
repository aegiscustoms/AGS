...
st.markdown("""
    <style>
        ...
        /* 3. 우측 상단 통합 레이아웃 (유저명 + 버튼) */
        .aegis-top-right-zone {
            position: absolute;
            top: -42px; 
            right: 0px;
            display: flex;
        ...
        /* 4. Logout 버튼 스타일 (12px, Logout 텍스트 전용) */
        div[data-testid="stButton"] > button[key="top_logout_btn"] {
...
# 1. 메뉴 구성 로직
all_menu_list = ["🔍 HS검색", "📘 HS정보", "📊 통계부호", "🚚 통관진행정보", "🧮 세액계산기"]
...
# ==========================================
# [데이터 로드 및 발췌 로직 수정: 10단위 DB 추가]
# ==========================================

@st.cache_data
def load_hs_resources():
    # 1. 4단위 호 리스트 (Heading Index) 로드
    try:
        df_hs = pd.read_csv('knowledge_base/headings/HS_Headings_All.csv', dtype={'류': str, '번호': str})
    except Exception as e:
        st.error(f"4단위 CSV 로드 실패: {e}")
        df_hs = pd.DataFrame()

    # 2. 10단위 상세 리스트 (관세사님이 알려주신 경로로 수정)
    try:
        # 경로: knowledge_base/headings/HS_subHeadings_All.csv
        df_sub = pd.read_csv('knowledge_base/headings/HS_subHeadings_All.csv', dtype={'HS부호': str})
        # HS부호 10자리 포맷 맞추기 (앞에 0이 빠진 경우 대비)
        df_sub['HS부호'] = df_sub['HS부호'].str.zfill(10)
    except Exception as e:
        st.warning(f"10단위 상세 DB 로드 실패: {e}")
        df_sub = pd.DataFrame()
    ...
    return df_hs, df_sub, manual_text

# 전역 데이터 변수 로드 (3개 변수로 받음)
df_hs_master, df_sub_master, manual_source_txt = load_hs_resources()

def get_legal_ground(p_name, manual_txt, hs_df, sub_df):
    """
    4단위 호의 해설서 본문과 해당 호에 속하는 실제 10단위 코드 리스트를 함께 반환합니다.
    """
    if hs_df.empty or not p_name:
        return "참조 법령 정보 없음", "Unknown", ""
    ...
    if matched.empty:
        return "매칭되는 4단위 호를 찾을 수 없습니다.", "Unknown", ""
    
    h_code = matched.iloc[0]['번호'] # 예: 8517
    formatted_h = f"{h_code[:2]}.{h_code[2:]}" # 8517 -> 85.17
    
    # 2. 10단위 실제 코드 리스트 추출 (Sub-headings 필터링) ★추가★
    actual_codes_list = ""
    if not sub_df.empty:
        # 4단위 번호로 시작하는 모든 10단위 코드 추출
        relevant_codes = sub_df[sub_df['HS부호'].str.startswith(h_code)]
        if not relevant_codes.empty:
            # AI가 읽기 편하게 '코드: 품목명' 형태로 변환
            actual_codes_list = relevant_codes[['HS부호', '한글품목명']].to_string(index=False)

    # 3. 해설서 본문 발췌 로직 (기존 유지)
    ...
    if not start_match:
        return f"제{formatted_h}호의 용어 정보를 기반으로 분석하세요.", h_code, actual_codes_list
    ...
    return manual_txt[start_pos:end_pos], h_code, actual_codes_list

# --- [Tab 1] HS검색 (가이드북/관세율표 지식 통합 및 전문 분석) ---
with tabs[0]:
    # 1) 전문성 있는 아이콘과 타이틀 배치
    st.markdown(f"<div style='font-size: 18px; font-weight: bold; color: #1E3A8A; margin-bottom: 15px;'> 💻 AI 기반 HS Code 전문 분석 리포트 </div>", unsafe_allow_html=True)
    ...
    if st.button("AI 전문 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("⏳ AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    # [2-1] 법령 및 4단위 호 정보 발췌
                    legal_ground_text, matched_h_code, actual_codes_pool = get_legal_ground(p_name, manual_source_txt, df_hs_master, df_sub_master)

                    # [2-2] HS_subHeadings_All.csv를 통한 실제 HS Code 리스트 추출
                    actual_codes_text = ""

                    try:
                        # CSV 로드 및 HS부호를 10자리 문자열로 변환 (0 채우기)
                        df_sub = pd.read_csv("HS_subHeadings_All.csv")
                        df_sub['HS부호'] = df_sub['HS부호'].astype(str).str.zfill(10)
                        
                        # matched_h_code(4단위)로 시작하는 실제 코드들 필터링
                        relevant_sub = df_sub[df_sub['HS부호'].str.startswith(matched_h_code)]
                        if not relevant_sub.empty:
                            actual_codes_text = relevant_sub[['HS부호', '한글품목명']].to_string(index=False)
                        else:
                            # 4단위 매칭이 없을 경우 대비하여 전체 검색 (품명 기준)
                            actual_codes_text = df_sub[df_sub['한글품목명'].str.contains(p_name[:3], na=False)].head(20)[['HS부호', '한글품목명']].to_string(index=False)
                    except Exception as e:
                        actual_codes_text = "실제 코드 데이터베이스를 참조할 수 없습니다."
                    ...
                    6. (세번 확정) 정확히 동일한 품명의 4단위 호가 있는 경우 하위 6단위 호를 찾아 확정하며, 특계된 호가 없다면 제일 하위에 '기타' 호로 분류할 것.
                    ...
                    - "4. 예상 경합세번"은 추천 번호의 확률이 100%가 아닌 경우만 상위 2순위 또는 3순위까지 표시하세요.
    ...
    if st.session_state.ai_report_done:
        ...
        st.divider()
        st.markdown("#### 🤵 10단위 확인 및 추가 검토가 필요하신 경우, 전문 관세사에게 문의주세요.")
...
# --- 하단 푸터 (유저정보 및 Logout 통합형 - 높이 28px 교정) ---
st.divider()

# 레이아웃 비율 조정 (연락처/링크들 : 유저정보 및 로그아웃)
f_left, f_right = st.columns([6.5, 3.5])
...
with f_right:
    ...
    if st.session_state.get('logged_in', False):
        # 내부 컬럼으로 이름과 버튼 나란히 배치
        u_col1, u_col2 = st.columns([0.6, 0.4])
            ...
            st.markdown(f"""
                <div style='text-align: right; padding-top: 0px;'>
                    <div style='display: inline-flex; 
                                ...
                                background-color: #f8f9fa; padding: 0px 12px; 
