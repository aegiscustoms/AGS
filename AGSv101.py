        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인", use_container_width=True):
                ...
                # 2) 일반 고객사 체크
                    ...
                    # 입력한 비밀번호 암호화 (소문자로 통일)
                    input_pw_hash = hashlib.sha256(l_pw.encode()).hexdigest().lower()
                    
                    if not df_users.empty and 'ID' in df_users.columns:
                        # 비교를 위해 전체 데이터의 공백 제거 및 문자열 변환
                        df_check = df_users.copy()
                        df_check['ID'] = df_check['ID'].astype(str).str.strip()
                        df_check['Password'] = df_check['Password'].astype(str).str.strip().str.lower()
                        
                        # 일치하는 레코드 찾기
                        user_match = df_check[(df_check['ID'] == l_id) & (df_check['Password'] == input_pw_hash)]
                        
                        if not user_match.empty:
                            user_info = user_match.iloc[0]
                            # 상태가 '활성'인지 확인 (Status 컬럼이 있을 경우)
                            status = str(user_info.get('Status', '활성')).strip()
                            if status == '활성':
                                st.session_state.logged_in = True
                                st.session_state.user_id = l_id
                        ...
                        st.error("⚠️ 사용자 데이터베이스를 불러올 수 없습니다.")
    st.stop()
# ◆변경내역◆끝◆
