...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        .stButton > button {{ background-color: #1E3A8A !important; color: white !important; border-radius: 6px !important; font-weight: 600 !important; width: 100% !important; }}
    </style>
""", unsafe_allow_html=True)

# --- [47행 시작] 쿠키 매니저 초기화 및 세션 복구 로직 ---
cookie_manager = stx.CookieManager()

# ★새로고침 대응: 쿠키가 브라우저에서 서버로 전달될 때까지 0.6초간 대기 (필수)
if 'cookie_wait_done' not in st.session_state:
    time.sleep(0.6) 
    st.session_state.cookie_wait_done = True

# 브라우저에 저장된 아이디 쿠키 가져오기
saved_user_id = cookie_manager.get(cookie="aegis_user_id")

# 로그인 세션이 비어있을 때 쿠키를 통한 자동 복구 시도
if 'logged_in' not in st.session_state or not st.session_state.logged_in:
    if saved_user_id:
        # 1. 쿠키 아이디가 있다면 세션 강제 복구
        st.session_state.logged_in = True
        st.session_state.user_id = saved_user_id
        
        # 2. 새로고침으로 날아간 이름과 권한 정보를 시트에서 즉시 동기화
        try:
            conn_gs = st.connection("gsheets", type=GSheetsConnection)
            df_sync = conn_gs.read(spreadsheet=st.secrets["connections"]["gsheets"]["spreadsheet"], worksheet="users", ttl=0)
            ...
            df_sync['ID_TMP'] = df_sync['ID'].apply(clean_id_sync)
            u_info = df_sync[df_sync['ID_TMP'] == saved_user_id]
            
            if not u_info.empty:
                st.session_state.user_name = u_info.iloc[0].get('NAME', saved_user_id)
                st.session_state.is_admin = (saved_user_id == "aegis01210")
                raw_tabs = u_info.iloc[0].get('ACCESS_TABS', "🔍 HS검색, 📘 HS정보, 📊 통계부호, 📦 화물통관진행정보, 🧮 세액계산기")
...
def init_db():
    ...
    ca = conn_auth.cursor()
    ca.execute("CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, pw TEXT, name TEXT, is_approved INTEGER DEFAULT 0, is_admin INTEGER DEFAULT 0)")
...
def write_log(user_id, user_name, activity, detail="-"):
    try:
        ...
        new_log = pd.DataFrame([{"Time": time.strftime("%Y-%m-%d %H:%M:%S"), "ID": user_id, "Name": user_name, "Activity": activity, "Detail": detail}])
                    ...
                    with st.spinner("보안 인증 및 데이터 연결 중..."):
                        try:
                            ...
                            if input_id == "aegis01210" and input_pw == "dlwltm2025@":
                                ...
                                cookie_manager.set("aegis_user_id", input_id, key="admin_cookie") # 쿠키 저장
                                ...
                                if 'ID' in df_check.columns:
                                    ...
                                    if not user_match.empty:
                                        ...
                                        if input_pw_hash == db_pw:
                                            # ★동시접속 제한 체크 예시 (시트에 LOGIN_STATUS가 ON이면 차단)
                                            # if user_match.iloc[0].get('LOGIN_STATUS') == 'ON':
                                            #     st.error("❌ 이미 접속 중인 아이디입니다.")
                                            #     st.stop()
                                            ...
                                            cookie_manager.set("aegis_user_id", input_id, key="user_cookie_save")
...
# ★핵심 수정: 로그인이 확실히 true이고, 필요한 정보(user_id)가 세션에 있을 때만 실행
if st.session_state.get('logged_in') and 'user_id' in st.session_state:
    # 안전하게 사용자 이름 가져오기 (없으면 ID 표시)
    u_name_display = st.session_state.get('user_name', st.session_state.user_id)
    
    st.sidebar.markdown(f"### 👤 {st.session_state.user_id} ({u_name_display})")
    
    # 로그아웃 버튼 (중복 방지를 위해 고유 key 적용)
    if st.sidebar.button("로그아웃", key="sidebar_logout_final"): 
        st.session_state.logged_in = False
        # 쿠키 삭제 시도
        try:
...
# 2. 사용자의 권한에 따라 실제 표시할 메뉴 리스트 결정
if st.session_state.is_admin:
    # 관리자는 모든 메뉴 + 관리자 탭
    display_menu_names = all_menu_list + ["⚙️ 관리자"]
else:
    # 일반 유저는 로그인 시 세션에 저장된 권한(user_permissions)만 필터링
    # 만약 권한 정보가 없다면 기본적으로 전체 메뉴를 보여줌 (안전장치)
...
if display_menu_names:
    ...
    st.error("❌ 이용 가능한 메뉴 권한이 없습니다. 관리자에게 문의하세요.")
    st.stop()
# ★변경내역★끝★
