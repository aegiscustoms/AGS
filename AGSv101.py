...
def load_hs_resources():
    ...
    # 2. 10단위 상세 리스트 (Sub-Headings) 로드 ★추가★
    try:
        # 파일이 root 또는 지정된 경로에 있는지 확인 필요 (예: 'knowledge_base/HS_subHeadings_All.csv')
        df_sub = pd.read_csv('HS_subHeadings_All.csv', dtype={'HS부호': str})
        # HS부호 10자리 포맷 맞추기 (0 채우기)
        df_sub['HS부호'] = df_sub['HS부호'].str.zfill(10)
    except Exception as e:
        st.error(f"10단위 상세 CSV 로드 실패: {e}")
        df_sub = pd.DataFrame()
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("⏳ AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    9. 반드시 제공된 [실제 HS Code 리스트] 내에 존재하는 번호 중에서만 추천하세요. 존재하지 않는 번호를 생성하면 안 됩니다.
                    """
