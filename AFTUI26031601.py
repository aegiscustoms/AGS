...
- "※ 주의사항 ※" 폰트색은 빨간색("ED1C24")으로 설정해주세요.
...
"""
    ...
    # [3] 결과 출력 및 검토의뢰 (결과 유지 및 연락처 정보 추가)
    if st.session_state.ai_report_done:
        ...
        # 버튼 높이 조절을 위한 CSS 주입 (입력창 높이 약 38px에 맞춤)
        st.markdown("""
            <style>
            /* 버튼 높이와 입력창 높이를 일치시키고 상단 여백 보정 */
        ...
        # 상호명, 연락처, 전송버튼 3분할 배치 (label_visibility="collapsed"로 수평 정렬)
        col_u1, col_u2, col_u3 = st.columns([0.35, 0.45, 0.2])
        
        with col_u1:
            user_org = st.text_input("상호명 (또는 성함)", key="user_org_v3", placeholder="예: (주)이지스무역", label_visibility="collapsed")
        with col_u2:
            user_contact = st.text_input("연락처 (전화 또는 이메일)", key="user_contact", placeholder="예: 010-1234-5678 / jhlee@gmail.com", label_visibility="collapsed")
        with col_u3:
