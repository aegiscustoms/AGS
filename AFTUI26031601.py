        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 AI 전문 관세사가 최신 해설서 및 법령을 분석 중입니다..."):
                try:
                    ...
                    [분석 지침]
                    1. (기본원칙) 제공된 [HS 해설서 법령 원문]에 근거하여 통칙 제1호(호의 용어 및 주 규정)를 최우선 적용하고, 제2호부터 제6호까지 논리적으로 순차 적용할 것.
                    2. (원문 참조 및 인용) 분석 시 반드시 'HS-manual.txt'에서 발췌된 아래 [HS 해설서 법령 원문]의 구체적인 문구와 단어를 직접 인용하여 리포트를 작성할 것.
                    3. (4단위 검토) 발췌된 제{matched_h_code}호의 용어와 부 또는 류 주규정, 총설, 호 해설서를 중심으로 예상되는 4단위 호를 우선 검토하고, 제외 규정에 해당되는지 여부를 확인할 것.
                    4. (세번 확정) 정확히 동일한 품명의 4단위 호가 있는 경우 하위 6단위, 10단위 호를 찾아 확정하며, 특계된 호가 없다면 제일 하위에 '기타' 호로 분류할 것.
                    5. (경합 검토) 예상되는 호가 복수일 경우 본질적인 특성, 재질, 기능에 따라 우선순위를 결정할 것.
                    6. (무결성) 답변 양식을 절대 임의로 축약하지 말고, 지정된 출력 양식을 100% 준수할 것.
                    """
