...
# --- [Tab 5] 세액계산기 (좌우 분할 및 행 최적화 레이아웃) --- #변경내역★★
with tabs[4]:
    # CSS: 13px 폰트 및 스타일 미세 조정
    st.markdown("""
        <style>
            ...
            .v5-title {
                font-size: 14px; font-weight: bold; margin-bottom: 10px; color: #1E3A8A;
            }
            /* 버튼과 입력창 높이 정렬 */
            div[data-testid="stButton"] button {
                height: 38px !important; margin-top: 0px !important;
            }
        </style>
    """, unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        with col_left:
            ...
            with l_row1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_inv_l")
            with l_row1_2:
                p_ex = st.number_input("적용환율 (KRW)", value=st.session_state.api_ex_rate, format="%.2f", key="v5_ex_rate_l")

            # (1-2) 중간행: 국가 및 통화코드, 구분, 환율조회버튼, 결과창
            st.write("국가 및 통화코드")
            l_row2_1, l_row2_2, l_row2_3, l_row2_4 = st.columns([1.5, 0.8, 1, 1])
            with l_row2_1:
                sel_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_l")
                target_curr = sel_curr.split(" ")[0]
            with l_row2_2:
                ex_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_l")
                imex_tp = "2" if ex_mode == "수입" else "1"
            with l_row2_3:
                if st.button("환율조회", use_container_width=True, key="v5_btn_l"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
            ...
            with l_row3_1:
                p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_l")
            with l_row3_2:
                p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_l")
        ...
        with col_right:
            ...
            hs_in = h1.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_r", placeholder="HS코드 10자리 입력")
            if h2.button("적용", key="v5_hs_btn_r"):
                if hs_in:
            ...
            with r_row2_1:
                a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_r")
            with r_row2_2:
                a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_r")

            # (2-3) 하단: 과세표준 출력 (CIF KRW)
            cif_val = int((p_price * p_ex) + p_frt + p_ins)
            st.write("과세표준 (CIF KRW)")
            st.markdown(f"""
                <div style='background-color:#EBF5FF; padding:10px; border-radius:5px; text-align:right; 
                font-size:15px; font-weight:bold; color:#1E40AF; border:1px solid #BFDBFE;'>
                    {cif_val:,.0f} 원
                </div>
            """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            
    # [4] 세액 계산 실행 및 상세 내역 테이블 (기존 동일 유지)
    if st.button("세액 계산 실행", use_container_width=True, type="primary"):
        d_amt = int(cif_val * (a_d/100))
        v_amt = int((cif_val + d_amt) * (a_v/100))
        
        st.markdown(f"<div style='font-size: 20px; font-weight: bold; color: #B91C1C; text-align: right; background-color: #FEF2F2; padding: 12px; border-radius: 8px; margin-bottom:15px;'>💰 예상세액 합계: {d_amt + v_amt:,.0f} 원</div>", unsafe_allow_html=True)
        
        # 결과 테이블 (생략 - 이전과 동일한 HTML 구조)
