...
# --- [Tab 5] 세액계산기 (상단 높이 정렬 및 스타일 최종 보정) --- #변경내역★★
with tabs[4]:
    ...
    with st.container(border=True):
        ...
        with col_left:
            ...
            # (1-1) 상단 첫행: 물품가격, 적용환율 (라벨 수정 및 정렬)
            l_row1_1, l_row1_2 = st.columns(2)
            with l_row1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_inv_f")
            with l_row1_2:
                # 적용환율 (KRW) -> 적용환율 수정
                p_ex = st.number_input("적용환율", value=st.session_state.api_ex_rate, format="%.2f", key="v5_ex_f")

            # (1-2) 중간행: 국가 및 통화코드 행
            st.write("국가 및 통화코드")
            l_row2_1, l_row2_2, l_row2_3, l_row2_4 = st.columns([1.5, 0.8, 1, 1])
            with l_row2_1:
                sel_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_f")
                target_curr = sel_curr.split(" ")[0]
            with l_row2_2:
                ex_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_f")
                imex_tp = "2" if ex_mode == "수입" else "1"
            with l_row2_3:
                if st.button("환율조회", use_container_width=True, key="v5_btn_f"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
            ...
            with l_row3_1:
                p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_f")
            with l_row3_2:
                p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_f")
        ...
        with col_right:
            ...
            # (2-1) 상단: 품목분류 (높이 정렬을 위해 padding 또는 위젯 구조 조정)
            st.write("품목분류 (HSK)")
            h1, h2 = st.columns([0.75, 0.25])
            with h1:
                hs_in = st.text_input("HSK 입력창", label_visibility="collapsed", key="v5_hs_f", placeholder="HS코드 10자리")
            with h2:
                # 버튼 높이를 입력창 라인에 맞춤
                apply_hs = st.button("적용", key="v5_hbtn_f", use_container_width=True)
                if apply_hs and hs_in:
            ...
            with r_row2_1:
                a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_f")
            with r_row2_2:
                a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_f")

            # (2-3) 하단: 과세표준 출력 (디자인 수정: 파란색 볼트체)
            cif_val = int((p_price * p_ex) + p_frt + p_ins)
            st.write("과세표준 (CIF KRW)")
            st.markdown(f"""
                <div style='background-color:#F0F9FF; padding:10px; border-radius:5px; text-align:right; 
                font-size:15px; font-weight:bold; color:#1D4ED8; border:1px solid #BAE6FD;'>
                    {cif_val:,.0f} 원
                </div>
            """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    # [4] 세액 계산 실행 버튼 및 테이블
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_exec_f"):
