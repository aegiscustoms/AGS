...
# --- [Tab 1] HS검색 (UI 정밀 보정 및 의뢰 기능 강화) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with col_left:
            st.write("**📸 물품 이미지**")
            uploaded_file = st.file_uploader("이미지 파일을 선택하세요", type=["jpg", "jpeg", "png"], key="hs_img_v3", label_visibility="collapsed")
            if uploaded_file:
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🧠 AI 전문관세사가 상세 분석 중입니다..."):
                try:
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    
                    # 프롬프트 고도화 (가독성을 위한 줄바꿈 지시 포함)
                    prompt = f"""당신은 30년 경력의 베테랑 관세사입니다. 제공된 정보를 바탕으로 다음 'AI 분석 리포트 양식'에 맞춰 분석 리포트를 작성하세요.
...
- "4. 예상 경합세번" 은 추천 번호의 확률이 100%가 아닌 경우만 표시하고, 좌측부터 상위 3순위까지 추천하되 3순위가 없으면 2순위까지만 표시.

[AI 분석 리포트 작성시 주의사항]
- 리포트내 항목명과 항목내용사이에 한줄띄우기를 적용하지 말고, 이전항목내용과 다음항목명 사이에는 반드시 빈 줄 한 줄을 추가하여 가독성을 높여주세요.
- "2. 추천 HS Code (10자리): XXXX.XX-XXXX (예상 확률 %)"에서 "XXXX.XX-XXXX"는 20px로 설정해주세요.
- "💡 AI 분석 리포트", "※ 주의사항 ※" 폰트는 20px로 설정해주세요.
- 각 항목명의 폰트는 반드시 "굵게" 해주세요
- 각 항목명 하위정보의 글머리는 "1)"로 넘버링하거나 "-" 또는 "•" 으로 해주세요.
...
"""
    ...
    # [3] 결과 출력 및 검토의뢰 (Label 복원 및 버튼 수직 확장)
    if st.session_state.ai_report_done:
        ...
        # 버튼을 항목명(Label) + 입력창 높이에 맞추기 위한 CSS 정밀 조정
        st.markdown("""
            <style>
            /* 버튼의 컨테이너를 하단 정렬이 아닌 상단부터 꽉 차게 설정 */
            ...
            /* 버튼 자체의 높이를 Label+Input 높이(약 68px)에 맞춰 확장 */
            div[data-testid="stButton"] > button {
                height: 68px !important;  /* Label(약 25px) + Input(38px) + 간격 고려 */
                margin-top: 28px !important; /* Label 높이만큼 내려오지 않고 정렬 맞춤용 */
                width: 100% !important;
        ...
        # 상호명, 연락처, 전송버튼 배치 (Label 복원)
        col_u1, col_u2, col_u3 = st.columns([0.35, 0.45, 0.2])
        
        with col_u1:
            user_org = st.text_input("상호명 (또는 성함)", key="user_org_v3", placeholder="예: (주)이지스무역")
        with col_u2:
            user_contact = st.text_input("연락처 (전화 또는 이메일)", key="user_contact", placeholder="예: 010-1234-5678 / jhlee@gmail.com")
        with col_u3:
            # 버튼 상단의 빈 공간을 채우기 위한 더미 텍스트(Label과 높이 맞춤용)
            st.markdown("<div style='margin-top: 25px;'></div>", unsafe_allow_html=True) 
            send_click = st.button("관세사 검토의뢰", use_container_width=True, type="secondary")
        
        if send_click:
            if not user_org or not user_contact:
                st.error("⚠️ 상호명과 연락처를 모두 입력해주세요.")
            else:
                import smtplib
                ...
                if not SENDER_PASSWORD:
                    st.error("❌ 메일 발송 설정 오류 (Secrets 확인 필요)")
                else:
                    msg = MIMEMultipart()
...
"""
                    ...
                    if st.session_state.last_img_bytes:
                        try:
                            ...
                            img_attachment.add_header('Content-Disposition', 'attachment', filename=f'request_{user_org}.png')
                    ...
                    try:
                        ...
                        server.quit()
                        st.success(f"✅ 의뢰 접수 완료! 등록하신 연락처({user_contact})로 곧 연락드리겠습니다.")
                    except Exception as e:
