...
# --- [Tab 5] 세액계산기 (디자인 고정 및 면책 문구 추가본) --- #변경내역★★
with tabs[4]:
    ...
    with st.container(border=True):
        ...
        with c_left:
            ...
            with l_r2[0]:
                ...
                t_curr = s_curr.split(" ")[0]
            with l_r2[1]:
                e_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_final")
                im_tp = "2" if e_mode == "수입" else "1"
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_api_final"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
                        q_dt = time.strftime("%Y%m%d")
                        url = "https://unipass.customs.go.kr:38010/ext/rest/trifFxrtInfoQry/retrieveTrifFxrtInfo"
                        params = {"crkyCn": API_KEY, "qryYymmDd": q_dt, "imexTp": im_tp}
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                for item in items:
                                    if item.findtext("currSgn") == t_curr:
                                        r_val = float(item.findtext("fxrt").replace(",", ""))
        ...
        with c_right:
            ...
            with h1: hs_in = st.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_final", placeholder="HS 10자리")
            with h2:
