...
# --- [Tab 2] HS정보 (디자인 정밀 보정 및 기능 복구) --- #변경내역★★
with tabs[1]:
            ...
            try:
                ...
                if not m.empty or not std.empty:
                    ...
                    if not r_all.empty:
                        ...
                        # [핵심] 테이블 양쪽 맞춤 및 Sticky Header 인라인 스타일 정의 #변경내역★★
                        def styled_rate_table(df, is_scroll=False):
                            table_id = "sticky-header-table" if is_scroll else "full-width-table"
                            # CSS: min-width 100%로 양쪽 맞춤, th에 sticky 적용하여 헤더 고정 #변경내역★★
                            custom_css = f"""
                            <style>
                                #{table_id} {{ width: 100% !important; border-collapse: collapse !important; table-layout: fixed !important; }}
                                #{table_id} th {{ 
                                    position: sticky !important; top: 0 !important; z-index: 10 !important;
                                    background-color: #F8FAFC !important; color: #1E3A8A !important; 
                                    text-align: center !important; padding: 12px !important; 
                                    border: 1px solid #E2E8F0 !important; font-size: 15px !important;
                                }}
                                #{table_id} td {{ text-align: center !important; padding: 10px !important; border: 1px solid #F1F5F9 !important; font-size: 13px !important; }}
                            </style>
                            """
                            html = df.to_html(index=False, justify='center', table_id=table_id)
                            # 10cm(378px) 고정폭 유지
                            colgroup = '<colgroup><col style="width: 378px;"><col style="width: auto;"><col style="width: 378px;"></colgroup>'
                            styled_html = html.replace('<thead>', colgroup + '<thead>')
                            
                            if is_scroll:
                                return f'{custom_css}<div style="max-height: 450px; overflow-y: auto; width: 100%; border: 1px solid #E2E8F0; border-radius: 4px;">{styled_html}</div>'
                            else:
                                return f'{custom_css}<div style="width: 100%;">{styled_html}</div>'
                        ...
                        st.markdown("— 협정세율 (FTA) [상위 10개 표시/항목명 고정 스크롤]") 
                        st.write(styled_rate_table(rf, is_scroll=True), unsafe_allow_html=True)
                    
                    st.divider()
                    st.markdown("**🛡️ 세관장확인대상 (수출입요건)**") 
                    
                    # [보정] 요건 테이블 항목명 가운데 정렬 및 양쪽 맞춤 #변경내역★★
                    def styled_req_table(df):
                        req_css = """
                        <style>
                            .req-table { width: 100% !important; border-collapse: collapse !important; }
                            .req-table th { text-align: center !important; background-color: #F8FAFC !important; color: #1E3A8A !important; padding: 12px !important; border: 1px solid #E2E8F0 !important; }
                            .req-table td { text-align: center !important; padding: 10px !important; border: 1px solid #F1F5F9 !important; }
                        </style>
                        """
                        html = df.to_html(index=False, classes="req-table", justify='center')
                        return f'{req_css}<div style="width: 100%;">{html}</div>'
