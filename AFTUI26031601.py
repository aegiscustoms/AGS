...
with tabs[3]:
    ...
    # --- 2. 우편물(EMS) 조회 로직 (정밀 코드 및 연결 안정화 적용) ---
    if post_btn:
        ...
        if not POST_KEY: st.error("❌ 우편물 API 키(POST_API_KEY) 설정 누락.")
            ...
            with st.spinner(f"{p_label} 정보 조회 중..."):
                ...
                try:
                    ...
                    st.error(f"❌ 우편물 조회 오류: {e} (서버 연결이 불안정합니다. 잠시 후 다시 시도해주세요.)")
