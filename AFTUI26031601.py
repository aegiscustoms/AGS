    ...
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                ...
                with st.expander("➕ 신규 화주 등록 및 탭 권한 설정"):
                    c1, c2 = st.columns(2)
                    new_id = c1.text_input("아이디(ID)", key="new_u_id")
                    new_nm = c2.text_input("성명/업체명", key="new_u_nm")
                    access_tabs = st.multiselect("접근 허용 기능", ["HS정보(Tab2)", "수입요건(Tab3)", "통관정보(Tab4)", "세액계산(Tab5)"], default=["통관정보(Tab4)"])
                    if st.button("계정 생성 및 권한 부여"):
                        st.success(f"✅ {new_nm} 계정 생성 완료")

                st.divider()
                st.write("**📊 현재 등록 계정 현황**")
                try:
                    conn = sqlite3.connect("customs_master.db")
                    u_df = pd.read_sql("SELECT id, name, role FROM users", conn)
                    st.dataframe(u_df, use_container_width=True, hide_index=True)
                    target_del = st.selectbox("삭제 대상 선택", u_df["id"].tolist())
                    if st.button("계정 삭제", type="primary"):
                        st.warning(f"⚠️ {target_del} 삭제됨")
                    conn.close()
                except: st.info("사용자 데이터베이스가 비어있습니다.")
