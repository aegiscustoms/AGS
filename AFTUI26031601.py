...
"""
    ...
    if st.session_state.ai_report_done:
        ...
        # 버튼 위치 및 높이를 상단 라벨과 입력창 전체 높이에 맞추는 정밀 CSS
        st.markdown("""
            <style>
            /* 1. 버튼 자체의 높이를 입력창(38px)보다 크게 하여 전체 영역을 채움 */
            div[data-testid="stButton"] > button {
                height: 40px !important;
                margin-top: 24px !important; /* 라벨(Label)의 평균 높이만큼 아래로 밀어 수평 맞춤 */
                width: 100% !important;
                background-color: #1E3A8A !important;
                color: white !important;
                border: none !important;
            }
            /* 2. 컬럼 내부의 정렬을 바닥(Basline)으로 강제 고정 */
            [data-testid="column"] {
                display: flex;
        ...
        # 컬럼 비율 조정 (버튼이 너무 길지 않게 0.25로 소폭 조정)
        ...
        with col_u3:
            # 버튼 클릭 시 실행될 로직
            send_click = st.button("관세사 검토의뢰", use_container_width=True)
...
"""
                    ...
                    try:
                        ...
                        server.quit()
                        st.success(f"✅ 의뢰 접수 완료! ({user_contact})")
                    except Exception as e:
                        st.error(f"❌ 전송 실패: {e}")
