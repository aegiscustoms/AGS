...
# --- [Tab 1] HS검색 (표준해석지침 지식 통합 및 전문 분석 버전) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        st.markdown("**📋 물품 정보 입력 (관세청 표준해석지침 및 관세율표 기반 정밀 분석)**")
    ...
    if st.button("AI 전문 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 지식 베이스 및 통계 조회 중..."):
                try:
                    # [2-1] 통계 API 호출 로직 (안전 모드 유지)
                    stats_data = ""
                    ...
                    # [2-2] 제미나이 하이브리드 지식 분석
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    
                    # 지식 베이스 지침 강화
                    knowledge_instruction = """
                    [수석 관세사 분석 지침]
                    1. 관세청 발간 '산업별 HS 표준해석 지침(반도체, 자동차부품, 이차전지, 디스플레이, 화장품, 직구 100선)'의 논리를 최우선 적용할 것.
                    2. 관세율표 제16부(84, 85류) 및 제17부(87류) 주 규정을 엄격히 해석할 것. 특히 부분품 분류 시 주 제2호 적용 프로세스를 서술할 것.
                    3. 분류 근거 작성 시 "관세율표 제00부 주 제0호" 또는 "표준해석지침에 따라"와 같은 전문적 인용구를 사용할 것.
                    """

                    prompt = f"""당신은 30년 경력의 베테랑 관세사입니다. {knowledge_instruction}
                    
                    [관세청 실제 신고 통계]
                    {stats_data}

                    [리포트 작성 지침]
                    - 리포트 내 항목명과 항목내용 사이에는 줄바꿈을 하지 말고, 이전 항목 내용과 다음 항목명 사이에는 반드시 빈 줄을 추가하세요.
                    - 각 항목명(1., 2., 3., 4.)은 **굵게** 표시하세요.
                    - 아래 특정 문구는 HTML 태그를 사용하세요:
                      1) "💡 AI 분석 리포트" -> 20px
                      2) "2. 추천 HS Code" 내의 "XXXX.XX-XXXX" -> 20px
                      3) "※ 주의사항 ※" -> 20px 및 빨간색(#ED1C24)
                    ...
                    **2. 추천 HS Code (10자리)**: <span style='font-size:20px; font-weight:bold;'>XXXX.XX-XXXX</span> (예상 확률 %) 
                       - 4단위: XXXX [명칭]
                       - 6단위: XXXX.XX [명칭]
                       - 10단위: XXXX.XX-XXXX [명칭]

                    **📊 관세청 신고 통계 (최근 다빈도 신고 번호)**
                    {stats_data if stats_data else '통계 정보를 불러올 수 없어 AI 분석으로 대체합니다.'}

                    **3. 분류 근거 (3개 문단 구조)**
                       - (지침서와 주 규정을 인용한 전문적 분석 서술)

                    **4. 예상 경합세번**
                       - 6단위/10단위 상위 후보군

                    <span style='font-size:20px; font-weight:bold; color:#ED1C24;'>※ 주의사항 ※</span>
                    - 실제 물품 스펙에 따라 변동 가능함. 법적 책임 없음. 상세 문의는 하단 의뢰 버튼 활용.
                    """
                    
                    response = model.generate_content([prompt, Image.open(uploaded_file)] if uploaded_file else [prompt])
                    ...
                    if "1) 이미지 해석:" in response.text:
                        st.session_state.img_analysis_text = response.text.split("1) 이미지 해석:")[1].split("2) 품명:")[0].strip()
                    st.rerun()
    ...
    if st.session_state.ai_report_done:
        ...
        st.markdown("""
            <style>
            div[data-testid="stButton"] > button { height: 40px !important; margin-top: 24px !important; width: 100% !important; background-color: #1E3A8A !important; color: white !important; }
