...
# --- [Tab 1] HS검색 (결과 유지 및 메일 전송 보정) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with c3: p_function = st.text_input("4. 기능 및 작동원리", placeholder="예: 블루투스 통신, 유압식", key="p_function")
        with c4: p_component = st.text_input("5. 성분 및 함량", placeholder="예: 소고기 60%, 에탄올 70%", key="p_component")
        with c5: p_spec = st.text_input("6. 규격 및 사양", placeholder="예: 220V, 500GB, 15인치", key="p_spec")
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("제미나이가 전문 자료를 분석 중입니다..."):
                try:
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    prompt = f"당신은 전문 관세사입니다. 아래 물품 정보를 바탕으로 HS Code 10자리를 추천하고 근거를 상세히 설명하는 리포트를 작성하세요.\n\n[물품 정보]\n{input_summary}"
                    content = [prompt]
                    if uploaded_file:
                    ...
                    st.rerun() # 상태 업데이트 후 화면 갱신
                except Exception as e:
                    st.error(f"❌ 분석 중 오류 발생: {e}")

    # [3] 결과 출력 및 검토의뢰 영역 (세션 상태가 True일 때만 유지) #변경내역★★
    if st.session_state.ai_report_done:
        ...
        user_org = col_req1.text_input("상호명 (또는 성함)", placeholder="예: (주)이지스무역 / 홍길동", key="user_org_v2")
        ...
        if col_req2.button("관세사 검토의뢰", use_container_width=True):
            if not user_org:
                ...
                if not SENDER_PASSWORD:
                    st.error("❌ 시스템 설정 오류: Secrets에 EMAIL_PASSWORD를 설정해주세요.")
                else:
                    msg = MIMEMultipart()
