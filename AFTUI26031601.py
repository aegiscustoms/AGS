...
# --- [Tab 1] HS검색 (가이드북/관세율표 지식 통합 및 전문 분석) --- #변경내역★★
with tabs[0]:
    ...
    # [2] 분석 실행 로직 (통계 API + 지식 파일 실시간 참조) #변경내역★★
    if st.button("AI 전문 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 관세청 지침서 및 통계 데이터를 분석 중입니다..."):
                try:
                    # [2-1] 관세청 HS CODE 내비게이션 API (안전 모드)
                    stats_data = ""
                    ...
                    # [2-2] 제미나이 멀티모달 분석 (첨부파일 지식 활용)
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    
                    # 수석 관세사 분석 지침 (프롬프트 고도화)
                    knowledge_instruction = """
                    [수석 관세사 분석 지침]
                    1. 4단위 호의 용어(Heading)를 우선 확정할 것.
                    2. 제공된 '관세율표.docx' 및 산업별 'HS 표준해석 지침(반도체, 자동차, 이차전지, 디스플레이, K뷰티, 해외직구)'의 내용을 실제 근거로 사용할 것.
                    3. 통칙 제1호(호의 용어 및 주 규정)를 최우선 적용하고, 제2호부터 제6호까지 논리적으로 순차 적용할 것.
                    4. 부분품 분류 시 제16부 주 제2호 가,나,다 규정 및 각 산업별 지침서의 부분품 판정 사례를 인용할 것.
                    5. 답변 양식을 절대 임의로 축약하지 말고, 지정된 출력 양식을 100% 준수할 것.
                    """

                    prompt = f"""당신은 30년 경력의 베테랑 관세사입니다. 다음 양식에 맞춰 분석 리포트를 작성하세요. {knowledge_instruction}
                    ...
                    - 아래 특정 문구는 HTML 태그를 사용하여 서식을 고정하세요:
                      ...
                      3) "※ 주의사항 ※" -> 15px 및 빨간색(#ED1C24) 굵게
                    - "4. 예상 경합세번"은 추천 번호의 확률이 100%가 아닌 경우만 상위 3순위까지 표시.
                    ...
                    **1. 입력정보**
                       1) 이미지 해석: (이미지가 있다면 시각적 특징과 식별 포인트 상세 서술, 없으면 '정보 없음')
                    {input_summary}
                    ...
                    **3. 분류 근거 (3개 문단 구조)**
                       - (문단 1) 관세율표 부/류 주 규정 및 해설서 총설의 핵심 내용 서술 (관세율표.docx 내용 인용)
                       - (문단 2) 산업별 표준해석 지침서의 유사 사례 및 호의 용어 설명 인용 (제공된 PDF 지침 내용 인용)
                       - (문단 3) 통칙 제1호 및 제6호를 적용하여 최종 번호가 산출되는 논리 결론
                    ...
                    **4. 예상 경합세번**
                    ...
                    - 본 리포트는 제공된 정보와 관세청 표준해석 지침에 기초하여 작성되었으나, 실제 물품 스펙에 따라 변경될 수 있습니다.
                    - 본 리포트는 참고 자료이며 법적 책임을 지지 않습니다. 정확한 분류는 반드시 관세사에게 문의하십시오.
                    - 상세 상담이 필요하신 경우 하단 "관세사 검토의뢰"를 누르시면 본 리포트가 전문 관세사에게 전달됩니다.
                    """
                    
                    # 3단계: 멀티모달 입력 (텍스트 + 업로드 이미지 + 지식 베이스 파일)
                    # 실제 서비스 시에는 위에서 언급한 파일들을 content 리스트에 포함하여 전달합니다.
                    content = [prompt]
                    if uploaded_file:
