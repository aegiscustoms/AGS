...
# --- [Tab 4] 통관 진행 정보 (화물 & 우편물 통합: 최종 안정화 버전) ---
with tabs[3]:
    ...
    if post_btn:
        ...
        if not POST_KEY: st.error("❌ 우편물 API 키(POST_API_KEY) 설정 누락.")
        elif not psmt_no_raw: st.warning("⚠️ 우편물 번호를 입력해주세요.")
        else:
            with st.spinner("우편물 정보 조회 중..."):
                ...
                for kcd in ["01", "02"]:
                    try:
                        params = {"crkyCn": POST_KEY, "psmtKcd": kcd, "psmtNo": clean_no}
                        ...
                        if res.status_code == 200:
                            ...
                            if items:
                                ...
                                error_log.append(f"유형[{kcd}]: {msg}")
                        time.sleep(1.0)
                    except Exception as e:
                        error_log.append(f"유형[{kcd}]: 연결 오류 ({str(e)})")
                
                if found_data: st.rerun()
                else:
                    st.warning("⚠️ 우편물 정보를 찾을 수 없습니다.")
                    with st.expander("원인 확인"):
                        for log in error_log: st.write(log)

    # --- 3. 결과 출력 영역 ---
    # [3-1] 일반화물 출력 (상세 단계 NameError 수정 완료)
...
# --- [Tab 5] 세액계산기 (과세표준 디자인 최종 보정) ---
with tabs[4]:
    # CSS: 기본 폰트 크기 및 높이 균형 설정
    st.markdown("""
        <style>
            .v5-container label, .v5-container input, .v5-container div {
    ...
    with st.container(border=True):
        ...
        # --- 1) 왼쪽 영역: 가격 및 환율 ---
        with col_left:
            st.markdown("<div class='v5-container'>", unsafe_allow_html=True)
            l_row1_1, l_row1_2 = st.columns(2)
            with l_row1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_p_price_v10")
            with l_row1_2:
                p_ex = st.number_input("적용환율", value=st.session_state.api_ex_rate, format="%.2f", key="v5_p_ex_v10")

            st.write("국가 및 통화코드")
            l_row2_1, l_row2_2, l_row2_3, l_row2_4 = st.columns([1.5, 0.8, 1, 1])
            with l_row2_1:
                sel_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_v10")
                target_curr = sel_curr.split(" ")[0]
            with l_row2_2:
                ex_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_v10")
                imex_tp = "2" if ex_mode == "수입" else "1"
            with l_row2_3:
                if st.button("환율조회", use_container_width=True, key="v5_api_btn_v10"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
                        q_date = time.strftime("%Y%m%d")
                        url = "https://unipass.customs.go.kr:38010/ext/rest/trifFxrtInfoQry/retrieveTrifFxrtInfo"
                        params = {"crkyCn": API_KEY, "qryYymmDd": q_date, "imexTp": imex_tp}
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                for item in items:
                                    if item.findtext("currSgn") == target_curr:
                                        rate_val = float(item.findtext("fxrt").replace(",", ""))
                                        st.session_state.api_ex_rate = rate_val
                                        st.session_state.api_ex_disp = f"{rate_val:,.2f}"
                                        st.rerun()
                        except: pass
            with l_row2_4:
                disp_val = st.session_state.api_ex_disp if st.session_state.api_ex_disp else "결과"
                st.markdown(f"<div style='background-color:#F1F5F9; padding:7px; border-radius:4px; text-align:center; font-size:13px; font-weight:bold; height:38px; line-height:24px; border:1px solid #CBD5E1;'>{disp_val}</div>", unsafe_allow_html=True)

            l_row3_1, l_row3_2 = st.columns(2)
            with l_row3_1: p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_v10")
            with l_row3_2: p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_v10")
        ...
        with col_right:
            ...
            with h1:
                hs_in = st.text_input("HSK 입력창", label_visibility="collapsed", key="v5_hs_v10", placeholder="HS코드 10자리")
            with h2:
                if st.button("적용", key="v5_hbtn_v10", use_container_width=True):
                    if hs_in:
                        hsk_clean = re.sub(r'[^0-9]', '', hs_in).zfill(10)
                        conn = sqlite3.connect("customs_master.db")
                        r_df = pd.read_sql(f"SELECT rate FROM rates WHERE hs_code = '{hsk_clean}' AND type IN ('A', 'C')", conn)
                        conn.close()
                        if not r_df.empty:
                            st.session_state.calc_d = float(pd.to_numeric(r_df['rate']).min())
                            st.rerun()

            r_row2_1, r_row2_2 = st.columns(2)
            with r_row2_1: a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_v10")
            with r_row2_2: a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_v10")

            # --- [과세표준 출력부: 강제 디자인 적용] ---
            cif_val = int((p_price * p_ex) + p_frt + p_ins)
            # 라벨에도 스타일을 직접 입힘
            st.markdown("<div style='font-size:13px; color:#1E3A8A; font-weight:bold; margin-top:5px; margin-bottom:5px;'>과세표준 (CIF KRW)</div>", unsafe_allow_html=True)
            
            # 박스와 텍스트 색상/굵기를 최우선으로 강제 고정
            st.markdown(f"""
                <div style='background-color: #F0F9FF !important; padding: 10px !important; border-radius: 5px !important; text-align: right !important; border: 1px solid #BAE6FD !important;'>
                    <span style='color: #1D4ED8 !important; font-size: 17px !important; font-weight: 900 !important; font-family: "Pretendard", sans-serif !important; display: inline-block !important;'>
                        {cif_val:,.0f} 원
                    </span>
                </div>
            """, unsafe_allow_html=True)
    ...
    # [4] 세액 계산 실행
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_exec_v10"):
        d_amt = int(cif_val * (a_d/100))
        v_amt = int((cif_val + d_amt) * (a_v/100))
        ...
        t_style = 'style="width:100%; border-collapse:collapse; border:1px solid #E2E8F0;"'
        th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; text-align:center; padding:10px; border:1px solid #E2E8F0; font-size:13px;"'
        td_c = 'style="text-align:center; padding:8px; border:1px solid #F1F5F9; font-size:13px;"'
        td_r = 'style="text-align:right; padding:8px; border:1px solid #F1F5F9; font-size:13px; font-weight:600;"'
        
        st.write(f"""
        <table {t_style}>
            <thead><tr><th {th_style}>세종</th><th {th_style}>산출근거</th><th {th_style}>세액(원)</th></tr></thead>
            <tbody>
                <tr><td {td_c}>관세</td><td {td_c}>{cif_val:,.0f} x {a_d}%</td><td {td_r}>{d_amt:,.0f}</td></tr>
                <tr><td {td_c}>부가세</td><td {td_c}>({cif_val:,.0f} + {d_amt:,.0f}) x {a_v}%</td><td {td_r}>{v_amt:,.0f}</td></tr>
            </tbody>
        </table>
        """, unsafe_allow_html=True)
