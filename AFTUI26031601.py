...
# --- [Tab 1] HS검색 (좌우 분할 레이아웃 및 전문 리포트) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with col_right:
            ...
            p_component = st.text_input("5. 성분 및 함량", key="p_component", placeholder="예: 소고기 60%, 화학물질명 등")
    ...
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            st.warning("⚠️ 분석을 위해 이미지 업로드 또는 최소 하나 이상의 정보를 입력해주세요.")
        else:
            with st.spinner("AI 전문 관세사가 상세 분석 중입니다..."):
                try:
                    model = genai.GenerativeModel('gemini-2.0-flash')
                    prompt = f"""당신은 30년 경력의 베테랑 관세사입니다. 다음 '제미나이 리포트 양식'에 맞춰 분석 리포트를 작성하세요.
...
"""
                    ...
                    try:
                        ...
                        server.quit()
                        st.success(f"✅ {user_org}님, 검토의뢰 메일이 전송되었습니다.")
                    except Exception as e:
