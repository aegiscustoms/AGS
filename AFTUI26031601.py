...
# --- [Tab 1] HS검색 (모델명 오류 수정 및 UI 개편) --- #변경내역★★
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        with c2:
            p_name = st.text_input("1. 품명 (기본)", placeholder="예: 무선 이어폰, 시스템 에어컨", key="p_name")
            p_material = st.text_input("2. 재질", placeholder="예: 스테인리스강, 폴리카보네이트 등", key="p_material")
            p_usage = st.text_input("3. 용도", placeholder="예: 가정용, 차량 부품용, 의료용 등", key="p_usage")

        c3, c4, c5 = st.columns(3)
        with c3:
            p_function = st.text_input("4. 기능 및 작동원리", placeholder="예: 블루투스 통신, 유압식", key="p_function")
        with c4:
            p_component = st.text_input("5. 성분 및 함량", placeholder="예: 소고기 60%, 에탄올 70%", key="p_component")
        with c5:
            p_spec = st.text_input("6. 규격 및 사양", placeholder="예: 220V, 500GB, 15인치", key="p_spec")
        
        p_composition = st.text_input("7. 구성요소 (Set 여부)", placeholder="예: 본체+케이블+매뉴얼 세트 등", key="p_composition")

    # [2] 분석 실행 로직
    if st.button("AI 분석 리포트 생성", use_container_width=True, type="primary"):
        # 입력 데이터 취합 (비어있는 항목은 제외)
        details = []
        if p_name: details.append(f"- 품명: {p_name}")
        if p_material: details.append(f"- 재질: {p_material}")
        if p_usage: details.append(f"- 용도: {p_usage}")
        if p_function: details.append(f"- 기능/원리: {p_function}")
        if p_component: details.append(f"- 성분/함량: {p_component}")
        if p_spec: details.append(f"- 규격/사양: {p_spec}")
        if p_composition: details.append(f"- 구성: {p_composition}")
        
        input_summary = "\n".join(details)
        
        if not uploaded_file and not input_summary:
            st.warning("⚠️ 분석을 위해 이미지 업로드 또는 최소 하나 이상의 물품 정보를 입력해주세요.")
        else:
            with st.spinner("제미나이가 전문 자료를 분석 중입니다..."):
                try:
                    # [수정] 모델명을 'gemini-1.5-flash-latest'로 변경하여 호환성 확보 #변경내역★★
                    model = genai.GenerativeModel('gemini-1.5-flash-latest')
                    
                    prompt = f"""당신은 전문 관세사입니다. 아래 제공된 물품 정보를 바탕으로 가장 적절한 HS Code 10자리를 추천하고 분류 근거를 상세히 설명하는 'HS 품목분류 분석 리포트'를 작성하세요.
                    
                    [물품 정보]
                    {input_summary}
                    
                    [리포트 작성 지침]
                    1. 예상 HS Code 10자리를 명시할 것.
                    2. 관세율표 해석 통칙 및 호의 용어를 바탕으로 분류 근거를 설명할 것.
                    3. 화주가 이해하기 쉽게 전문적이면서도 친절한 어조를 유지할 것.
                    """
                    ...
                    user_org = col_req1.text_input("상호명 (또는 성함)", placeholder="예: (주)이지스무역 / 홍길동", key="user_org")
                    ...
                    if col_req2.button("관세사 검토의뢰", use_container_width=True):
                        if not user_org:
                            ...
                            # Streamlit Secrets에서 앱 비밀번호 로드
                            SENDER_PASSWORD = st.secrets.get("EMAIL_PASSWORD", "") 

                            if not SENDER_PASSWORD:
                                st.error("❌ 시스템 설정 오류: 메일 발송용 비밀번호가 설정되지 않았습니다. (Secrets 확인 필요)")
                            else:
                                msg = MIMEMultipart()
                                ...
                                try:
                                    ...
                                    server.quit()
                                    st.success(f"✅ {user_org}님, 검토의뢰가 성공적으로 접수되었습니다.")
