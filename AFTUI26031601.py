...
# --- [DB 경로 고정: 관세사님 지정 경로] --- ★수정 반영
# r을 붙여 백슬래시(\) 경로 인식을 정확하게 합니다.
db_path = r"C:\Users\USER\Desktop\AGS\customs_master.db"

# 해당 경로에 폴더가 있는지 확인하고 없으면 에러를 띄우거나 생성합니다.
if not os.path.exists(r"C:\Users\USER\Desktop\AGS"):
    try:
        os.makedirs(r"C:\Users\USER\Desktop\AGS")
    except Exception as e:
        st.error(f"폴더 생성 실패. 경로를 확인해주세요: {e}")
        ...
        if not st.session_state.admin_verified:
            ...
            with adm_tabs[1]:
                ...
                with st.expander("➕ 신규 화주 등록 및 초기 비밀번호 설정", expanded=False):
                    ...
                    new_pw = c3.text_input("초기 비밀번호", type="password", value="1234", key="adm_new_pw")
                    
                    access_tabs = st.multiselect(
                        "접근 허용 기능", 
                        ["📘 HS정보", "📊 통계부호", "📦 화물통관진행정보", "🧮 세액계산기"], 
                        default=["📦 화물통관진행정보"]
                    )
                    
                    if st.button("계정 생성 실행", use_container_width=True):
                        if new_id and new_nm:
                            conn = sqlite3.connect(db_path) # 지정 경로 연결
                            ...
                            try:
                                ...
                                conn.commit()
                                st.success(f"✅ {new_nm}({new_id}) 계정 생성 완료. (DB 경로: {db_path})")
                                time.sleep(0.5); st.rerun()
                            except sqlite3.IntegrityError:
                                st.error("❌ 이미 존재하는 아이디입니다.")
                            finally: conn.close()
                        else: st.warning("ID와 성명을 입력하세요.")

                st.divider()

                st.write("**📊 현재 등록된 계정 현황**")
                try:
                    conn = sqlite3.connect(db_path) # 지정 경로 연결
                    u_df = pd.read_sql("SELECT id, name, access_tabs FROM users WHERE is_admin=0", conn)
                    
                    if not u_df.empty:
                        st.dataframe(u_df, use_container_width=True, hide_index=True)
                        
                        c_sel, c_btn = st.columns([1.5, 2])
                        with c_sel:
                            target_user = st.selectbox("관리 대상 선택", u_df["id"].tolist(), key="sel_target")
                        
                        with c_btn:
                            st.write(" ") 
                            b_c1, b_c2 = st.columns(2)
                            if b_c1.button("🔑 비번 1234 초기화", use_container_width=True):
                                reset_pw = hashlib.sha256("1234".encode()).hexdigest()
                                conn.execute("UPDATE users SET pw = ? WHERE id = ?", (reset_pw, target_user))
                                conn.commit()
                                st.info(f"💡 {target_user} 비번 초기화 완료.")
                            
                            if b_c2.button("🗑️ 계정 영구 삭제", type="primary", use_container_width=True):
                                conn.execute("DELETE FROM users WHERE id = ?", (target_user,))
                                conn.commit()
                                st.warning(f"⚠️ {target_user} 삭제 완료."); time.sleep(0.5); st.rerun()
                    else: st.info("등록된 화주 계정이 없습니다.")
                    conn.close()
