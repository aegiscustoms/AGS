...
import time
import os  # 경로 자동 인식을 위해 os 모듈 추가
...
# --- [DB 기초 공사: 바탕화면 AGS 경로 자동 인식] --- ★개선 반영
def get_db_path():
    try:
        # 윈도우 사용자명을 수동 수정할 필요 없이 현재 사용자의 바탕화면 경로를 자동으로 탐색합니다.
        desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
        target_dir = os.path.join(desktop_path, "AGS")
        
        # 바탕화면에 AGS 폴더가 없으면 자동으로 생성합니다.
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)
            
        return os.path.join(target_dir, "customs_master.db")
    except Exception as e:
        # 보안 문제로 바탕화면 접근이 막힐 경우, 프로그램 실행 폴더에 생성하도록 예외 처리
        return "customs_master.db"
...
# --- 1. 초기 DB 설정 (통합 DB로 최적화 및 원본 테이블 보존) --- #변경내역★★
def init_db():
    # [통합] 바탕화면 AGS 폴더의 customs_master.db에 모든 정보 저장
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS users (
                ...
                access_tabs TEXT DEFAULT 'ALL')""")
    
    # 관리자 계정 초기 생성 (dlwltm2025@)
    admin_id = "aegis01210"
    admin_pw = hashlib.sha256("dlwltm2025@".encode()).hexdigest()
    c.execute("INSERT OR IGNORE INTO users (id, pw, is_approved, is_admin, name) VALUES (?, ?, 1, 1, ?)", (admin_id, admin_pw, "관리자"))
...
def get_legal_ground(p_name, manual_txt, hs_df):
    ...
    formatted_h = f"{h_code[:2]}.{h_code[2:]}"
...
if st.session_state.get('is_admin', False):
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            # --- [서브탭 2: 계정 및 권한 관리 - 통합 DB 실시간 연동 버전] ---
# --- [서브탭 2: 계정 및 권한 관리 - 통합 DB 실시간 연동 버전] ---
            with adm_tabs[1]:
                st.subheader("👤 화주 계정 상세 제어")
                
                # 1. 신규 계정 생성 (초기 비밀번호 설정 및 DB 저장 로직)
                with st.expander("➕ 신규 화주 등록 및 초기 비밀번호 설정", expanded=False):
                    c1, c2, c3 = st.columns([1, 1, 1])
                    new_id = c1.text_input("아이디(ID)", key="adm_new_id")
                    new_nm = c2.text_input("성명/업체명", key="adm_new_nm")
                    # 초기 비밀번호 설정 (화면 표시용)
                    new_pw = c3.text_input("초기 비밀번호", type="password", value="1234", help="기본값은 1234입니다.", key="adm_new_pw")
                    
                    access_tabs = st.multiselect(
                        "접근 허용 기능", 
                        ["HS정보(Tab2)", "수입요건(Tab3)", "통관정보(Tab4)", "세액계산(Tab5)"], 
                        default=["통관정보(Tab4)"]
                    )
                    
                    if st.button("계정 생성 실행", use_container_width=True):
                        if new_id and new_nm:
                            ...
                            try:
                                cursor.execute("""
                                    INSERT INTO users (id, pw, name, is_approved, is_admin, access_tabs) 
                                    VALUES (?, ?, ?, 1, 0, ?)
                                """, (new_id, hashed_pw, new_nm, str(access_tabs)))
                                conn.commit()
                                st.success(f"✅ {new_nm}({new_id}) 계정이 생성되었습니다. (초기 PW: {new_pw})")
                                time.sleep(1)
                                st.rerun() # 목록 갱신
                            ...
                            st.warning("⚠️ ID와 성명을 입력해주세요.")

                st.divider()

                # 2. 계정 목록 및 비밀번호 초기화 (통합 DB 연동)
                st.write("**📊 현재 등록된 계정 현황 및 보안 관리**")
                try:
                    # 통합 DB(db_path)에 연결
                    conn = sqlite3.connect(db_path)
                    # 관리자 제외 화주 목록만 로드
                    u_df = pd.read_sql("SELECT id, name, is_approved, access_tabs FROM users WHERE is_admin=0", conn)
                    ...
                    if not u_df.empty:
                        ...
                        with c_del1:
                            target_user = st.selectbox("관리 대상 계정 선택", u_df["id"].tolist(), key="sel_target")
                        ...
                        with c_del2:
                            ...
                            with sub_c1:
                                if st.button("🔑 비밀번호 초기화", use_container_width=True):
                                    # [실제 DB 업데이트 로직: 비밀번호를 '1234'로 변경]
                                    cursor = conn.cursor()
                                    reset_pw_hash = hashlib.sha256("1234".encode()).hexdigest()
                                    cursor.execute("UPDATE users SET pw = ? WHERE id = ?", (reset_pw_hash, target_user))
                                    conn.commit()
                                    st.info(f"💡 {target_user}의 비밀번호가 '1234'로 초기화되었습니다.")
                            
                            with sub_c2:
                                if st.button("🗑️ 계정 삭제", type="primary", use_container_width=True):
                                    # [실제 DB 삭제 로직]
                                    cursor = conn.cursor()
                                    cursor.execute("DELETE FROM users WHERE id = ?", (target_user,))
                                    conn.commit()
                                    st.warning(f"⚠️ {target_user} 계정이 삭제되었습니다.")
                                    time.sleep(1)
                                    st.rerun()
                    else:
                        st.info("등록된 화주 계정이 없습니다.")
                    conn.close()
                except Exception as e:
...
# [중요] 앱 최상단 혹은 Tab 1 진입 전 점검모드 로직 추가 권장
if st.session_state.get('maintenance_mode', False) and not st.session_state.get('is_admin', False):
    st.warning("🚧 현재 시스템 점검 중입니다. 이용에 불편을 드려 죄송합니다.")
    st.stop()
