...
# --- [Tab 4] 화물통관 (테이블 디자인 및 날짜 포맷 최적화) --- #변경내역★★
with tabs[3]:
    ...
    if st.session_state.search_result:
        ...
        if info is not None:
            ...
            # [보정] 날짜 포맷 변경 함수 (20260310134104 -> 2026.03.10_13:41:04) #변경내역★★
            ...
            m4.metric("현재위치", latest_location[:15])
            
            # [보정] 상세 진행 단계 테이블 (양쪽 맞춤, 헤더 고정, 날짜 포맷팅) #변경내역★★
            if dtls:
                st.write("**📑 상세 진행 단계**")
                
                # 인라인 스타일 정의 (가운데 정렬, 헤더 고정, 너비 100%)
                ...
                # 열 너비 비율 (단계 25%, 일시 35%, 장소 40%)
                html += '<colgroup><col style="width:25%;"><col style="width:35%;"><col style="width:40%;"></colgroup>'
                html += f'<thead><tr><th {th_style}>처리단계</th><th {th_style}>처리일시</th><th {th_style}>장치장/내용</th></tr></thead>'
                html += '<tbody>'
                
                for item in dtls:
                    step = get_safe_text(item, "cargTrcnRelaBsopTpcd")
                    # 날짜 포맷 적용 #변경내역★★
                    raw_dt = get_safe_text(item, "prcsDttm")
                    proc_dt = format_date(raw_dt)
