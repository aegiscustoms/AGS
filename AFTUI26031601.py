...
# --- [Tab 4] 화물통관 (특수문자 제거 및 자동완성 로직 보정) --- #변경내역★★
with tabs[3]:
    ...
    with c_b: 
        bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v6").strip().upper()
    with c_m: 
        mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v6").strip()
    ...
    if st.button("실시간 조회 실행", use_container_width=True):
        ...
        if not API_KEY:
            ...
            with st.spinner("관세청 유니패스 데이터 조회 중..."):
                ...
                # [강화] 화물관리번호 정제: 하이픈 제거 및 대문자 변환 #변경내역★★
                ...
                if clean_mrn: params["cargMtNo"] = clean_mrn # 정제된 번호 송신
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if info is not None and info.findtext("prgsStts"):
                            ...
                            st.warning("⚠️ 일치하는 화물 진행 정보가 없습니다. 번호나 입항년도를 다시 확인해주세요.")
                    else:
                        st.error(f"❌ 유니패스 응답 오류 ({res.status_code})")
                except Exception as e:
                    st.error(f"❌ 통신 오류: {e}")

    # 2. 결과 출력 영역 (기존 디자인 유지)
