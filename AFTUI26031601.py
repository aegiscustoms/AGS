...
# --- [Tab 4] 통관 진행 정보 (화물 & 우편물 통합: 최종 간소화 버전) ---
with tabs[3]:
    ...
    with st.container(border=True):
        ...
        with c_y: carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0, key="cargo_yy_v2")
        with c_b: bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v2").strip().upper()
        with c_m: mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v2").strip()
        with c_btn: 
            st.write(" ") # 간격 맞춤용
            cargo_btn = st.button("화물 조회", use_container_width=True, type="primary", key="btn_cargo_v2")

    # [B] 우편물 통관 검색 섹션 (EMS/우편물 번호 통합)
    st.subheader("✉️ 우편물 통관 (EMS/국제우편)")
    with st.container(border=True):
        p_n, p_btn = st.columns([3, 1])
        with p_n: 
            psmt_no = st.text_input("우편물 번호 (13자리)", placeholder="예: EE123456789KR", key="post_no_v2").strip().upper()
        with p_btn:
            st.write(" ") # 간격 맞춤용
            post_btn = st.button("우편물 조회", use_container_width=True, type="primary", key="btn_post_v2")
    ...
    # --- 2. 우편물(EMS) 조회 로직 (우편물 종류 자동화: 변경내역★★★★) ---
    if post_btn:
        ...
        if not POST_KEY: st.error("❌ 우편물 API 키(POST_API_KEY)가 설정되지 않았습니다.")
            ...
            with st.spinner("유니패스 우편물 정보 조회 중..."):
                ...
                # 번호 정제
                clean_psmt_no = re.sub(r'[^a-zA-Z0-9]', '', psmt_no).upper()
                
                # 우선 EMS('01')로 시도하고 결과가 없으면 기타('02')로 자동 재시도하는 로직
                found_data = False
                for kcd in ["01", "02"]:
                    params = {"crkyCn": POST_KEY, "psmtKcd": kcd, "psmtNo": clean_psmt_no}
                    try:
                        res = requests.get(url, params=params, timeout=10)
                        if res.status_code == 200:
                            root = ET.fromstring(res.content)
                            # 에러 메시지 확인
                            err_msg = root.findtext(".//ntceMsgCn")
                            items = root.findall(".//psmtCsclPrgsInfoQryRsltVo")
                            
                            if items and not err_msg:
                ...
                if found_data:
                    ...
                    st.warning("⚠️ 일치하는 우편물 정보가 없습니다. 번호를 다시 확인하시거나, 아직 통관 진행 전인지 확인해주세요.")

    # --- 3. 결과 출력 영역 ---
    # [3-1] 화물 결과 출력 (기본 테이블 로직)
    if st.session_state.last_search_type == "CARGO" and st.session_state.cargo_result:
        ...
        if info is not None:
            ...
            # (이후 상세 테이블 출력 생략 - 기존 코드 동일)

    # [3-2] 우편물 결과 출력 (가이드 기반 최적화)
    if st.session_state.last_search_type == "POST" and st.session_state.post_result:
        ...
        if items:
            st.divider()
            st.success(f"✅ 우편물 번호 조회 결과")
            latest = items[0]
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("최신단계", latest.findtext("psmtPrcsTpcdNm") or "")
            m2.metric("처리기관", latest.findtext("csclPsofNm") or "")
            m3.metric("발송국가", latest.findtext("sendCntyCdNm") or "")
            m4.metric("총중량", latest.findtext("ttwg") or "0")

            st.write("**📋 우편물 통관 진행 이력**")
            post_data = []
            for item in items:
                dt_raw = item.findtext("brngArvlDt") or ""
                formatted_dt = f"{dt_raw[0:4]}.{dt_raw[4:6]}.{dt_raw[6:8]}" if len(dt_raw) >= 8 else dt_raw
                post_data.append({
