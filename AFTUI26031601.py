...
각 주요 번호(1, 2, 3, 4) 항목 사이에는 반드시 빈 줄 한 줄을 추가하여 가독성을 높이세요.
...
3. 분류 근거 (다음 3개 문단 구조를 지킬 것)
   - [문단 1]: 물품이 속하는 부/류의 분류 규정(주 규정) 및 해설서 총설의 핵심 내용 서술
   - [문단 2]: 확정된 4단위 호의 용어 설명 및 호해설서에서 해당 물품을 분류할 수 있는 구체적 근거 예시 인용
   - [문단 3]: 물품의 특징(재질/기능 등)을 요약하고 통칙 제1호 및 제6호를 적용하여 최종 번호가 산출되는 논리 결론
...
"""
    ...
    if st.session_state.ai_report_done:
        ...
        # 상호명, 연락처, 전송버튼 3분할 배치
        col_u1, col_u2, col_u3 = st.columns([0.4, 0.4, 0.2])
        user_org = col_u1.text_input("상호명 (또는 성함)", key="user_org_v3", placeholder="예: (주)이지스무역")
        user_contact = col_u2.text_input("연락처 (전화 또는 이메일)", key="user_contact", placeholder="예: 010-1234-5678")
        
        if col_u3.button("검토의뢰 전송", use_container_width=True, type="secondary"):
