...
import time # 상단에 import time이 없다면 추가해주세요

# --- [Tab 4] 통관 진행 정보 (화물 & 우편물 통합: 출력 복구 및 안정화) ---
with tabs[3]:
    ...
    with st.container(border=True):
        ...
        with c_y: carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0, key="cargo_yy_v4")
        with c_b: bl_input = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search_v4").strip().upper()
        with c_m: mrn_input = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search_v4").strip()
        with c_btn: 
            st.write(" ") 
            cargo_btn = st.button("화물 조회", use_container_width=True, type="primary", key="btn_cargo_v4")

    # [B] 우편물 통관 검색 섹션 (EMS/국제우편)
    st.subheader("✉️ 우편물 통관 (EMS/국제우편)")
    with st.container(border=True):
        p_n, p_btn = st.columns([3, 1])
        with p_n: 
            psmt_no_raw = st.text_input("우편물 번호 (13자리)", placeholder="예: EE123456789KR", key="post_no_v4").strip().upper()
        with p_btn:
            st.write(" ") 
            post_btn = st.button("우편물 조회", use_container_width=True, type="primary", key="btn_post_v4")

    # --- 1. 일반화물 조회 로직 ---
    if cargo_btn:
        # 시크릿 키 명칭 확인: UNIPASS_API_KEY 또는 CARGO_API_KEY
        PI_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
            ...
            with st.spinner("유니패스 우편물 정보 조회 중..."):
                ...
                for kcd in ["01", "02"]:
                    try:
                        ...
                        # 연속 호출 전 짧은 대기 (Connection Reset 방지)
                        time.sleep(0.5)
                    except Exception as e:
    ...
    if st.session_state.last_search_type == "CARGO" and st.session_state.cargo_result:
        ...
        if info is not None:
            ...
            def get_t(node, tag, length=None):
                v = node.findtext(tag) or ""
                return v[:length] if length else v

            m1, m2, m3, m4 = st.columns(4)
            m1.metric("현재상태", get_t(info, "prgsStts"))
            m2.metric("품명", get_t(info, "prnm", 12))
            m3.metric("중량", f"{get_t(info, 'ttwg')} {get_t(info, 'wghtUt')}")
            loc = dtls[0].findtext("shedNm") or dtls[0].findtext("rlbrCn") if dtls else "정보없음"
            m4.metric("현재위치", loc[:15])

            if dtls:
                st.write("**📑 상세 진행 단계**")
                # 테이블 서식 적용
                t_style = 'style="width:100%; border-collapse:collapse; border:1px solid #E2E8F0;"'
                th_style = 'style="background-color:#F8FAFC; color:#1E3A8A; padding:10px; border:1px solid #E2E8F0; font-size:13px;"'
                td_style = 'style="padding:10px; border:1px solid #F1F5F9; font-size:12px; text-align:center;"'
                
                html = f'<table {t_style}><thead><tr><th {th_style}>처리단계</th><th {th_style}>처리일시</th><th {th_style}>장치장/내용</th></tr></thead><tbody>'
                for item in dtls:
                    proc_dt = get_t(item, "prcsDttm")
                    fmt_dt = f"{proc_dt[0:4]}.{proc_dt[4:6]}.{proc_dt[6:8]} {proc_dt[8:10]}:{proc_dt[10:12]}" if len(proc_dt)>=12 else proc_dt
                    html += f'<tr><td {td_style}>{get_t(item, "cargTrcnRelaBsopTpcd")}</td><td {td_style}>{fmt_dt}</td><td {td_style}>{get_t(item, "shedNm") or get_t(item, "rlbrCn")}</td></tr>'
                html += '</tbody></table>'
                st.write(f'<div style="max-height:400px; overflow-y:auto;">{html}</div>', unsafe_allow_html=True)
