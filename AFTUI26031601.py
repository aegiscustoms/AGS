...
# --- [Tab 5] 세액계산기 (UI 균형 및 과세표준 디자인 최종 보정) --- #변경내역★★
with tabs[4]:
    # CSS: 13px 폰트 및 상단 행 높이 균형을 위한 스타일
    st.markdown("""
        <style>
            .v5-container label {
    ...
    with st.container(border=True):
        ...
        with col_left:
            ...
            with l_row1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_p_price_v9")
            with l_row1_2:
                p_ex = st.number_input("적용환율", value=st.session_state.api_ex_rate, format="%.2f", key="v5_p_ex_v9")

            # (1-2) 중간 행: 환율 조회 통합 라인
            st.write("국가 및 통화코드")
            l_row2_1, l_row2_2, l_row2_3, l_row2_4 = st.columns([1.5, 0.8, 1, 1])
            with l_row2_1:
                sel_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_v9")
                target_curr = sel_curr.split(" ")[0]
            with l_row2_2:
                ex_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_v9")
                imex_tp = "2" if ex_mode == "수입" else "1"
            with l_row2_3:
                if st.button("환율조회", use_container_width=True, key="v5_api_btn_v9"):
                    API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
                    if API_KEY:
            ...
            with l_row3_1:
                p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_v9")
            with l_row3_2:
                p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_v9")
        ...
        with col_right:
            ...
            # (2-1) 상단 행: 품목분류 (좌측 물품가격 행과 높이 맞춤)
            st.write("품목분류 (HSK)")
            h1, h2 = st.columns([0.75, 0.25])
            with h1:
                hs_in = st.text_input("HSK 입력창", label_visibility="collapsed", key="v5_hs_v9", placeholder="HS코드 10자리")
            with h2:
                if st.button("적용", key="v5_hbtn_v9", use_container_width=True):
                    if hs_in:
            ...
            with r_row2_1:
                a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_v9")
            with r_row2_2:
                a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_v9")

            # (2-3) 하단 행: 과세표준 출력 (파란색/볼트체 스타일 강제 적용)
            cif_val = int((p_price * p_ex) + p_frt + p_ins)
            st.write("과세표준 (CIF KRW)")
            
            # 인라인 스타일을 활용해 텍스트 증발 방지 및 강조 극대화
            st.markdown(f"""
                <div style="background-color: #F0F9FF; padding: 10px; border-radius: 5px; border: 1px solid #BAE6FD; text-align: right;">
                    <span style="color: #1D4ED8 !important; font-weight: 800 !important; font-size: 16px !important; font-family: 'Pretendard', sans-serif !important; display: block;">
                        {cif_val:,.0f} 원
                    </span>
                </div>
            """, unsafe_allow_html=True)
    ...
    # [4] 세액 계산 실행 결과 (이전과 동일)
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_exec_v9"):
