...
## --- [Tab 4] 통관 진행 정보 (화물 & 우편물 통합: 실무 최적화 버전) ---
with tabs[3]:
    st.markdown("<div class='custom-header'>📦 실시간 통관 진행 정보 조회 (AEGIS)</div>", unsafe_allow_html=True)
    ...
    # [B] 우편물 통관 검색 섹션 (특급 기본값 및 코드 숨김 반영) ★
    st.subheader("✉️ 우편물 통관 (EMS/국제우편)")
    with st.container(border=True):
        p_t, p_n, p_btn = st.columns([1.5, 2, 1])
        with p_t:
            # 화면에는 한글만 표시, 내부적으로는 코드 매핑
            p_type_map = {
                "특급 (EMS)": "14", 
                "항공소포": "13", 
                "항공등기": "12",
            ...
            p_labels = list(p_type_map.keys())
            # "특급 (EMS)"가 리스트의 0번째이므로 기본값으로 자동 설정됨
            p_label = st.selectbox("우편물 종류", p_labels, index=0, key="post_type_select")
    ...
    if cargo_btn:
        ...
        if not API_KEY: st.error("❌ API 키 설정 누락.")
        else:
    ...
    # --- 2. 우편물(EMS) 조회 로직 (정밀 코드 적용) ---
    if post_btn:
        POST_KEY = st.secrets.get("POST_API_KEY", "").strip()
        if not POST_KEY: st.error("❌ 우편물 API 키 설정 누락.")
        elif not psmt_no_raw: st.warning("⚠️ 우편물 번호를 입력해주세요.")
        else:
            with st.spinner(f"{p_label} 정보 조회 중..."):
                ...
                try:
                    # 선택된 한글 라벨에 해당하는 정밀 코드(selected_kcd) 전송
    ...
    if st.session_state.last_search_type == "CARGO" and st.session_state.cargo_result:
        ...
        if info is not None:
            ...
            if dtls:
                st.write("**📑 상세 진행 단계**")
                t_html = '<table style="width:100%; border-collapse:collapse; border:1px solid #E2E8F0;">'
                t_html += '<tr style="background-color:#F8FAFC;"><th style="padding:10px; border:1px solid #E2E8F0;">단계</th><th style="padding:10px; border:1px solid #E2E8F0;">일시</th><th style="padding:10px; border:1px solid #E2E8F0;">장소</th></tr>'
                for d in dtls:
                    ...
                    t_html += f'<tr><td style="padding:8px; border:1px solid #F1F5F9; text-align:center; font-size:12px;">{d.findtext("cargTrcnRelaBsopTpcd") or ""}</td>'
                    t_html += f'<td style="padding:8px; border:1px solid #F1F5F9; text-align:center; font-size:12px;">{fmt_dt}</td>'
                    t_html += f'<td style="padding:8px; border:1px solid #F1F5F9; text-align:center; font-size:12px;">{d.findtext("shedNm") or d.findtext("rlbrCn") or ""}</td></tr>'
                t_html += '</table>'
                st.write(f'<div style="max-height:400px; overflow-y:auto;">{t_html}</div>', unsafe_allow_html=True)
    ...
    if st.session_state.last_search_type == "POST" and st.session_state.post_result:
        ...
        if items:
            ...
            top = items[0]
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("최신단계", top.findtext("psmtPrcsTpcdNm") or "-")
            m2.metric("우체국", top.findtext("csclPsofNm") or "-")
            m3.metric("발송국", top.findtext("sendCntyCdNm") or "-")
            m4.metric("중량", f"{top.findtext('ttwg') or '0'} kg")

            post_data = []
            for item in items:
                d = item.findtext("brngArvlDt") or ""
                post_data.append({
                    "진행단계": item.findtext("psmtPrcsTpcdNm"),
                    "장소": item.findtext("csclPsofNm"),
                    "날짜": f"{d[0:4]}.{d[4:6]}.{d[6:8]}" if len(d)>=8 else d,
                    "통관관리번호": item.findtext("psmtCsclMtNo")
                })
            st.table(pd.DataFrame(post_data))

# --- [Tab 5] 세액계산기 (환율조회 기능 복구 및 디자인 최종본) --- #변경내역★★
with tabs[4]:
    # CSS: 13px 폰트 및 과세표준 전용 최우선순위 스타일 정의
    st.markdown("""
        <style>
            .v5-font label, .v5-font input, .v5-font div {
                font-size: 13px !important;
            }
            .v5-title {
                font-size: 14px; font-weight: bold; margin-bottom: 12px; color: #1E3A8A;
            }
            /* 과세표준 박스 및 텍스트 강제 스타일 */
            .cif-box-final {
    ...
    with st.container(border=True):
        ...
        with c_left:
            ...
            with l_r1_1:
                p_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0, format="%.2f", key="v5_p_price_v11")
            with l_r1_2:
                p_ex = st.number_input("적용환율", value=st.session_state.api_ex_rate, format="%.2f", key="v5_p_ex_v11")

            st.write("국가 및 통화코드")
            l_r2 = st.columns([1.5, 0.8, 1, 1])
            with l_r2[0]:
                s_curr = st.selectbox("통화선택", currency_options, index=0, label_visibility="collapsed", key="v5_curr_v11")
                t_curr_code = s_curr.split(" ")[0]
            with l_r2[1]:
                e_mode = st.radio("구분", ["수입", "수출"], horizontal=True, label_visibility="collapsed", key="v5_mode_v11")
                im_tp_val = "2" if e_mode == "수입" else "1"
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_api_btn_v11"):
                    ...
                    if API_KEY:
                        ...
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                found = False
                                for item in items:
                                    # 가이드 응답 항목: currSgn(통화부호), fxrt(환율)
                                    if item.findtext("currSgn") == t_curr_code:
                                        ...
                                        if r_str:
                                            ...
                                            break
                                if not found:
                                    st.session_state.api_ex_disp = "N/A"
                        except:
                            st.session_state.api_ex_disp = "Error"
            with l_r2[3]:
                d_val = st.session_state.api_ex_disp if st.session_state.api_ex_disp else "결과"
                st.markdown(f"<div style='background-color:#F1F5F9; padding:7px; border-radius:4px; text-align:center; font-size:13px; font-weight:bold; height:38px; line-height:24px; border:1px solid #CBD5E1;'>{d_val}</div>", unsafe_allow_html=True)

            l_r3_1, l_r3_2 = st.columns(2)
            with l_r3_1: p_frt = st.number_input("운임 (Freight, KRW)", min_value=0, key="v5_frt_v11")
            with l_r3_2: p_ins = st.number_input("보험료 (Insurance, KRW)", min_value=0, key="v5_ins_v11")
        ...
        with c_right:
            ...
            with h1: hs_in = st.text_input("HSK 입력", label_visibility="collapsed", key="v5_hs_v11", placeholder="HS코드 10자리")
            with h2:
                if st.button("적용", key="v5_hbtn_v11", use_container_width=True):
                    if hs_in:
                        h_cl = re.sub(r'[^0-9]', '', hs_in).zfill(10)
                        conn = sqlite3.connect("customs_master.db")
                        r_df = pd.read_sql(f"SELECT rate FROM rates WHERE hs_code = '{h_cl}' AND type IN ('A', 'C')", conn)
                        conn.close()
                        if not r_df.empty:
                            st.session_state.calc_d = float(pd.to_numeric(r_df['rate']).min())
                            st.rerun()

            r_r2_1, r_r2_2 = st.columns(2)
            with r_r2_1: a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_v11")
            with r_r2_2: a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_v11")
    ...
    # [4] 세액 계산 실행 및 결과
    if st.button("세액 계산 실행", use_container_width=True, type="primary", key="v5_exec_v11"):
...
# --- [Tab 6] 관리자 모드 (기존 업로드 복구 + 신규 관리 기능 통합) --- #변경내역★★
if st.session_state.get('is_admin', False):
    with tabs[-1]:
        st.markdown("<div class='custom-header'>⚙️ 관리자 시스템 센터</div>", unsafe_allow_html=True)
            ...
            with adm_tabs[0]:
                ...
                for i, m_name in enumerate(m_list):
                    with cols[i%3]:
                        ...
                        up = st.file_uploader(f"{m_name} 업로드", type="csv", key=f"ad_{m_name}_f", label_visibility="collapsed")
                        
                        if up and st.button(f"반영", key=f"btn_{m_name}_f", use_container_width=True):
                            df = safe_read_csv(up) # 기존 정의된 safe_read_csv 함수 사용
                            if df is not None:
                                conn = sqlite3.connect("customs_master.db")
                                try:
                                    # [기존 매핑 로직 유지]
                                    if m_name == "HS코드(마스터)":
                                        ...
                                        target_table = "req_import" if "수입" in m_name else "req_export"

                                    if target_table:
                                        if 'hs_code' in df_map.columns:
                                            df_map['hs_code'] = df_map['hs_code'].astype(str).str.replace(r'[^0-9]', '', regex=True).str.zfill(10)
                                        df_map.to_sql(target_table, conn, if_exists='replace', index=False)
                ...
                for i, s_name in enumerate(stat_list):
                    with s_cols[i%2]:
                        ...
                        s_up = st.file_uploader(f"{s_name} 업로드", type="csv", key=f"sup_{s_name}_f", label_visibility="collapsed")
                        if s_up and st.button(f"{s_name} 반영", key=f"sbtn_{s_name}_f", use_container_width=True):
                            # (기존 통계부호 업로드 로직 동일 유지...)
                            st.session_state.upload_status[s_name] = True
                            st.success(f"✅ {s_name} 반영 완료")
                            st.rerun()

            # --- [서브탭 2: 사용자 계정 및 권한 관리] ---
            with adm_tabs[1]:
                st.subheader("👤 화주 계정 관리")
                try:
                    conn = sqlite3.connect("customs_master.db")
                    u_df = pd.read_sql("SELECT id, name, role, last_login FROM users", conn)
                    st.dataframe(u_df, use_container_width=True)
                    with st.expander("➕ 신규 계정 추가"):
                        nc1, nc2 = st.columns(2)
                        new_id = nc1.text_input("아이디")
                        new_nm = nc2.text_input("성명/업체명")
                        if st.button("등록 실행"): st.success("등록되었습니다.")
                    conn.close()
                except: st.info("사용자 정보를 불러올 수 없습니다.")

            # --- [서브탭 3: 이용 로그 확인] ---
            with adm_tabs[2]:
                st.subheader("📋 시스템 이용 현황")
                log_data = [{"일시": "2026-03-18 17:15", "사용자": "admin", "활동": "데이터 업데이트"}]
                st.table(log_data)

            # --- [서브탭 4: API 상태 점검 도구] ---
            with adm_tabs[3]:
                st.subheader("🔌 유니패스 API 연결 진단")
                ck1, ck2 = st.columns(2)
                with ck1:
                    with st.container(border=True):
                        st.write("**💰 관세환율 API**")
                        if st.button("환율 API 테스트", key="ping_exch"):
                            K = st.secrets.get("EXCH_API_KEY", "").strip()
                            D = time.strftime("%Y%m%d")
                            URL = f"https://unipass.customs.go.kr:38010/ext/rest/trifFxrtInfoQry/retrieveTrifFxrtInfo?crkyCn={K}&qryYymmDd={D}&imexTp=2"
                            res = requests.get(URL, timeout=5)
                            if "존재하지 않는 인증키" in res.text: st.error("❌ 미승인 키")
                            elif res.status_code==200: st.success("✅ 정상")
                with ck2:
                    with st.container(border=True):
                        st.write("**📦 우편물조회 API**")
                        if st.button("우편물 API 테스트", key="ping_post"):
                            K = st.secrets.get("POST_API_KEY", "").strip()
                            URL = f"https://unipass.customs.go.kr:38010/ext/rest/psmtCsclPrgsInfoQry/retrievePsmtCsclPrgsInfo?crkyCn={K}&psmtKcd=01&psmtNo=EG123456789KR"
                            res = requests.get(URL, timeout=5)
                            if "존재하지 않는 인증키" in res.text: st.error("❌ 미승인 키")
                            elif res.status_code==200: st.success("✅ 정상")
