...
# --- [DB 기초 공사: 바탕화면 AGS 경로 설정] --- ★요청사항 반영
db_dir = r"C:\Users\USER\Desktop\AGS"
db_path = os.path.join(db_dir, "customs_master.db")
...
if st.session_state.get('is_admin', False):
    with tabs[-1]:
        ...
        if not st.session_state.admin_verified:
            ...
            # --- [서브탭 2: 계정 및 권한 관리 - 비밀번호 기능 강화 ★] ---
            with adm_tabs[1]:
                st.subheader("👤 화주 계정 상세 제어")
                
                # 1. 신규 계정 생성 (초기 비밀번호 설정 추가)
                with st.expander("➕ 신규 화주 등록 및 초기 비밀번호 설정", expanded=False):
                    ...
                    if st.button("계정 생성 실행", use_container_width=True):
                        if new_id and new_nm:
                            # [DB 저장 로직: id, name, password, role 등을 insert]
                ...
                try:
                    ...
                    with c_del1:
                        target_user = st.selectbox("관리 대상 계정 선택", u_df["id"].tolist() if not u_df.empty else ["없음"], key="sel_target")
                    ...
                    with c_del2:
                        ...
                        with sub_c1:
                            if st.button("🔑 비밀번호 초기화", use_container_width=True):
                                # [DB Update 로직: 해당 ID의 password를 '1234'로 변경]
