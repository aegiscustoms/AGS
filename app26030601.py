...
with tabs[0]:
    ...
    u_input = st.text_input("물품 정보 입력", key="hs_q")
...
with tabs[1]:
    ...
    if st.button("실시간 관세율 조회", use_container_width=True):
        if target_hs:
            with st.spinner("유니패스 연결 중..."):
                url = "https://unipass.customs.go.kr:38010/ext/rest/itemRateQry/retrieveItemRate"
                params = {"crkyCn": RATE_KEY, "hsCd": target_hs.strip()}
                try:
                    res = requests.get(url, params=params, headers=HTTP_HEADERS, timeout=20)
                    if res.text.strip():
                        root = ET.fromstring(res.content)
                        items = root.findall(".//itemRateQryVo")
                        if items:
                            data = [{"코드": i.findtext("tarfClsfCd"), "명칭": i.findtext("tarfNm"), "세율": f"{i.findtext('itrt')}%"} for i in items]
                            st.dataframe(pd.DataFrame(data), hide_index=True, use_container_width=True)
                        else: st.warning("데이터가 없습니다. (해당 코드에 등록된 세율이 없거나 인증키 승인 확인 필요)")
                    else: st.error("서버에서 빈 응답을 보냈습니다. 유니패스에서 '품목별관세율' API 승인 여부를 확인하세요.")
...
with tabs[2]:
    ...
    if st.button("부호 검색", use_container_width=True):
        ...
        try:
            ...
            else: st.error("서버에서 빈 응답을 보냈습니다. 유니패스에서 '공통코드' API 승인 여부를 확인하세요.")
        except Exception as e: st.error(f"연결 오류: {e}")

# --- [Tab 4] 화물통관진행정보 (관세사님 고정 코드) ---
with tabs[3]:
