...
if len(tabs) > 5:
    if not st.session_state.is_admin:
        ...
        with tabs[-1]:
            ...
            # --- [서브탭 1: 데이터 업로드 - 자동 컬럼 매핑 및 DB 저장] ---
                with adm_tabs[0]:
                    if "upload_status" not in st.session_state:
                        st.session_state.upload_status = {}

                    # 공통 DB 연결 함수
                    def save_to_db(df, table_name, display_name):
                        try:
                            conn = sqlite3.connect("customs_master.db")
                            # 기존 데이터를 지우고 새로 덮어쓰기
                            df.to_sql(table_name, conn, if_exists='replace', index=False)
                            conn.close()
                            st.session_state.upload_status[display_name] = True
                            st.success(f"✅ {display_name} 데이터가 DB에 정상 반영되었습니다.")
                            time.sleep(0.5)
                            st.rerun()
                        except Exception as e:
                    ...
                    for i, m_name in enumerate(m_list):
                        with m_cols[i%3]:
                            ...
                            if up and st.button(f"{m_name} 반영", key=f"btn_{m_name}_f", use_container_width=True):
                                df_upload = pd.read_csv(up)
                                
                                # [자동 매핑 로직]
                                if m_name == "HS코드(마스터)":
                                    df_upload = df_upload.rename(columns={'HS부호': 'hs_code', '한글품목명': 'name_kr', '영문품목명': 'name_en'})
                                    save_to_db(df_upload, "hs_master", m_name)
                                elif m_name == "표준품명":
                                    df_upload = df_upload.rename(columns={'HS부호': 'hs_code', '표준품명_한글': 'std_name_kr', '표준품명_영문': 'std_name_en'})
                                    save_to_db(df_upload, "standard_names", m_name)
                                elif m_name == "관세율":
                                    df_upload = df_upload.rename(columns={'품목번호': 'hs_code', '관세율구분': 'rate_type', '관세율': 'rate_val'})
                                    save_to_db(df_upload, "rates", m_name)
                                elif m_name == "관세율구분":
                                    save_to_db(df_upload, "rate_types", m_name)
                                elif m_name == "세관장확인(수입)":
                                    df_upload = df_upload.rename(columns={'HS부호': 'hs_code', '신고인확인법령코드명': 'law_name', '요건확인서류명': 'doc_name'})
                                    save_to_db(df_upload, "confirm_import", m_name)
                                elif m_name == "세관장확인(수출)":
                                    df_upload = df_upload.rename(columns={'HS부호': 'hs_code', '신고인확인법령코드명': 'law_name', '요건확인서류명': 'doc_name'})
                                    save_to_db(df_upload, "confirm_export", m_name)
                    ...
                    for i, s_name in enumerate(stat_list):
                        with s_cols[i%2]:
                            ...
                            if s_up and st.button(f"{s_name} 반영", key=f"sbtn_{s_name}_f", use_container_width=True):
                                df_stat = pd.read_csv(s_up)
                                
                                # 통계부호 전용 매핑 및 저장 로직
                                if s_name == "간이세율(2026)":
                                    df_stat = df_stat.rename(columns={'HS부호': 'gani_hs', '품명': 'gani_name', '세율': 'gani_rate'})
                                    save_to_db(df_stat, "stat_gani", s_name)
                                elif s_name == "관세감면부호(2026)":
                                    save_to_db(df_stat, "stat_reduction", s_name)
                                elif s_name == "내국세면세부호(2026)": # <--- 이 부분이 추가되었습니다.
                                    save_to_db(df_stat, "stat_internal_exempt", s_name)
                                elif s_name == "내국세율(2026)":
                                    save_to_db(df_stat, "stat_internal_tax", s_name)
                                else:
                                    save_to_db(df_stat, s_name.replace("(2026)","").strip(), s_name)
