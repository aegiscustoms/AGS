...
def get_legal_ground(p_name, manual_txt, hs_df, sub_hs_df): 
    ...
    if not sub_hs_df.empty:
        related_hsk = sub_hs_df[sub_hs_df['HS10'].str.startswith(h_code, na=False)]
        if not related_hsk.empty:
            for _, row in related_hsk.head(20).iterrows():
                raw_hsk = str(row['HS10'])
                # 포맷팅: 8517131000 -> 8517.13-1000
                f_hsk = f"{raw_hsk[:4]}.{raw_hsk[4:6]}-{raw_hsk[6:]}"
                hsk_list_text += f"- {f_hsk} [{row['품명_en']}]\n"
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    **4. 추천 HS Code내 분류가능한 HSK**: <span style='font-size:20px; font-weight:bold;'> 
