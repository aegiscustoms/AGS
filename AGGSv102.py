...
with tabs[0]:
    ...
    if match:
        ...
        hs6_query = hs6_display.replace(".", "") # 점 제거 (847010)
        
        st.divider()
        st.subheader(f"🔍 {hs6_display} 내 분류가능한 HSK 10단위 리스트")
        
        # 전역 변수 df_sub_hs_master에서 직접 검색
        if not df_sub_hs_master.empty:
            target_hsk = df_sub_hs_master[df_sub_hs_master['HS10'].str.startswith(hs6_query, na=False)]
            
            if not target_hsk.empty:
                display_df = target_hsk.copy()
                # 10단위 포맷팅 (0000.00-0000)
                display_df['HSK 번호'] = display_df['HS10'].apply(lambda x: f"{x[:4]}.{x[4:6]}-{x[6:]}")
                display_df = display_df.rename(columns={'name_kr': '한글 품명', 'name_en': '영문 품명'})
                
                # 표(Table)로 출력
                st.table(display_df[['HSK 번호', '한글 품명', '영문 품명']])
            else:
