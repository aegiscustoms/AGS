...
def get_legal_ground(p_name, manual_txt, hs_df, sub_hs_df): 
    ...
    if not sub_hs_df.empty:
        search_target = str(h_code).strip().zfill(4) # 4단위 기준 추출
        ...
        if not related_hsk.empty:
            for _, row in related_hsk.iterrows():
                hsk_val = row['HS10']
                n_kr = str(row['name_kr']).strip() if pd.notna(row['name_kr']) else ""
                n_en = str(row['name_en']).strip() if pd.notna(row['name_en']) else ""
                
                # AI에게 '코드 | 국문 [영문]' 형태로 재료를 던져줌
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 지침:
                        ...
                        3) 골라낸 데이터를 'XXXX.XX-XXXX 국문품명 [영문품명]' 형식으로 변환하여 모두 나열하세요.
