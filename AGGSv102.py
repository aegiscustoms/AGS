...
with tabs[1]:
    ...
    st.markdown(f"""
        <style>
            .title-text {{ font-size: 16px; font-weight: bold; color: #1E3A8A; margin-bottom: 10px; }}
            .rate-val {{ font-size: 25px; font-weight: bold; color: #D32F2F; }}
            ...
            .container-green {{ background-color: #F0FDF4; border-color: #22C55E; color: #166534; font-size: 16px; font-weight: 600; }}
            .container-blue {{ background-color: #EFF6FF; border-color: #3B82F6; color: #1E40AF; font-size: 16px; }}
    ...
    if target_hs != "0000000000":
        ...
        with col_r2:
            ...
            st.dataframe(fta_res, height=300, use_container_width=True, hide_index=True, column_config=rate_config)
        st.divider()
        ...
        # 통합공고 전용 10px 스타일 주입
        st.markdown("<style> div[data-testid='stExpander'] div[role='table'] { font-size: 10px !important; } </style>", unsafe_allow_html=True)
        ...
        with col_cp1:
            ...
            if not cp_im_res.empty:
                ...
                st.dataframe(cp_im_display.sort_values('법령명'), use_container_width=True, hide_index=True, column_config=cp_config)
        ...
        with col_cp2:
            ...
            if not cp_ex_res.empty:
                ...
                st.dataframe(cp_ex_display.sort_values('법령명'), use_container_width=True, hide_index=True, column_config=cp_config)
            else: st.write("조회된 수출요령 정보가 없습니다.")

# --- [Tab 3] 통계부호 (정밀 매핑 및 출력 정렬 보정) --- #변경내역★★
with tabs[2]:
    st.markdown(f"<style>.tab3-title {{ font-size: 16px !important; font-weight: bold; color: #1E3A8A; margin-bottom: 5px; }}</style>", unsafe_allow_html=True)
...
with f_left:
    ...
    with fl_col1:
        st.markdown(f"<div style='padding-top: 5px; font-size: 13px;'><b>📞 010-8859-0403 (이지스)</b></div>", unsafe_allow_html=True)
