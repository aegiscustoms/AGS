...
def get_legal_ground(p_name, manual_txt, hs_df, sub_hs_df): 
    ...
    h_code = str(matched.iloc[0]['번호']).strip() # 예: "9003"
    formatted_h = f"{h_code[:2]}.{h_code[2:]}"

    # [추가] 10단위 HSK 리스트 추출 및 포맷팅 (0000.00-0000 형식)
    hsk_list_text = ""
    if not sub_hs_df.empty:
        # 4단위 h_code(예: 9003)를 기준으로 관련 10단위를 모두 추출
        search_target = str(h_code).strip().zfill(4)
        related_hsk = sub_hs_df[sub_hs_df['HS10'].str.startswith(search_target, na=False)]
        
        if not related_hsk.empty:
            for _, row in related_hsk.iterrows():
                hsk_val = row['HS10']
                # 국문품명과 영문품명을 가져옴 (결측치 처리 포함)
                n_kr = str(row['name_kr']).strip() if pd.notna(row['name_kr']) else "품명없음"
                n_en = str(row['name_en']).strip() if pd.notna(row['name_en']) else "No Description"
                
                # AI에게 '코드 | 국문 [영문]' 형태로 전달
                hsk_list_text += f"CODE:{hsk_val} | DESC:{n_kr} [{n_en}]\n"
        else:
            hsk_list_text = "해당 호의 상세 HSK 데이터가 없습니다."
    ...
    if not start_match:
        # [수정] 3개 반환하도록 hsk_list_text 추가
        return f"제{formatted_h}호의 용어와 관련 주 규정을 기반으로 분석하세요.", h_code, hsk_list_text
    ...
    # 정상 경로 (3개 반환)
    return manual_txt[start_pos:end_pos], h_code, hsk_list_text
    ...
    if st.button("AI 전문 분석 리포트 생성", use_container_width=True, type="primary"):
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    # [2-1] 실시간 법령 및 호 정보 발췌
                    # HS_Headings_All.csv와 HS-manual.txt에서 분석에 필요한 원문 추출
                    legal_ground_text, matched_h_code, hsk_raw_data = get_legal_ground(
                        p_name if p_name else "정보없음", 
                        manual_source_txt, 
                        df_hs_master, 
                        df_sub_hs_master  # 4번째 인자 추가
                    )
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 지침:
                        1) "2. 추천 HS Code"에서 당신이 확정한 6단위 번호를 확인하세요.
                        2) 위 [참조용 HSK 리스트 데이터] 중에서 앞 6자리가 당신이 확정한 번호와 '완벽히 일치'하는 데이터만 골라내세요.
                        3) 골라낸 데이터를 'XXXX.XX-XXXX 국문품명 [영문품명]' 형식으로 변환하여 모두 나열하세요. 예시: "9003.11-0000 플라스틱으로 만든 것 [Of plastics]"
                        4) 일치하는 데이터가 리스트에 없다면 "제공된 데이터 내에 해당 6단위의 상세 HSK 정보가 없습니다."라고 적으세요.
                        5) 이 과정은 2번 항목의 판단 결과에 절대 영향을 주어서는 안 되며, 판단이 끝난 후 데이터를 찾는 용도로만 사용하세요.
                    - "5. 예상 경합세번"은 추천 번호의 확률이 100%가 아닌 경우만 상위 3순위까지 표시하세요.
                    ...
                    **4. 추천 HS Code내 분류가능한 HSK** 
                       - (위 지침에 따라 필터링된 HSK 리스트를 'XXXX.XX-XXXX 국문품명 [영문품명]' 형식으로 출력하세요)                       
...
with tabs[2]:
    ...
    STATICS_SHEET_URL = "https://docs.google.com/spreadsheets/d/1fYs-hPiysNO6M4vQoOJ4cJSzr7XNzeKssH-IfEBrkbw/edit?usp=sharing"

    STATICS_MAPPING = {
        "간이세율": "GANI",
        "내국세율": "DOMESTIC",
        "관세감면율": "REDUCTION_TARIFF",
        "내국세감면율": "REDUCTION_DOMESTIC"
    }
    
    # 화면에 보여줄 옵션 리스트 (가나다순 정렬)
    display_options = sorted(list(STATICS_MAPPING.keys()))

    # --- 2. 검색창 및 버튼 레이아웃 ---
    # s3_col1: 자동완성 검색창, s3_col2: 조회 버튼
    s3_col1, s3_col2 = st.columns([0.8, 0.2])
    
    with s3_col1:
        # st.selectbox는 타이핑 시 검색/자동완성 기능을 기본으로 제공합니다.
        selected_display_name = st.selectbox(
            "조회할 통계부호명을 입력하거나 선택하세요",
            options=[""] + display_options,
            index=0,
            key="statics_search_select",
            help="부호명을 타이핑하면 목록이 자동으로 필터링됩니다."
        )
        
    with s3_col2:
        st.write("") 
        st.write("") 
        statics_search_btn = st.button("📊 데이터 조회", key="btn_statics_run", use_container_width=True)

    # --- 3. 조회 로직 및 결과 출력 ---
    # 목록에서 선택이 변경되거나 버튼을 클릭했을 때 실행
    if statics_search_btn or (selected_display_name != ""):
        if selected_display_name:
            # 매핑 정보를 통해 시트호출
            target_sheet = STATICS_MAPPING.get(selected_display_name)
            
            with st.spinner(f"[{selected_display_name}] 정보를 불러오는 중..."):
                try:
                    conn_statics = st.connection("gsheets", type=GSheetsConnection)
                    df_statics = conn_statics.read(
                        spreadsheet=STATICS_SHEET_URL, 
                        worksheet=target_sheet,
                        ttl=3600 
                    )
                    
                    if df_statics is not None and not df_statics.empty:
                        st.success(f"✅ [{selected_display_name}] 조회 결과 (Data: {target_sheet})")
                        ...
                        st.markdown("</div>", unsafe_allow_html=True)
                        csv = df_statics.to_csv(index=False).encode('utf-8-sig')
                        st.download_button(
                            label="📥 결과 다운로드 (CSV)",
                            data=csv,
                            file_name=f"{selected_display_name}_조회결과.csv",
                            mime="text/csv",
                        )
                    else:
                        st.info(f"'{selected_display_name}' 시트에 데이터가 비어있습니다.")
                        
                except Exception as e:
                    st.error(f"오류 발생: 구글 시트 내에 '{target_sheet}' 시트가 존재하는지 확인해 주세요.")
        else:
            st.info("좌측 상단에서 조회할 통계부호를 선택해 주세요.")
