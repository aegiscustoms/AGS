            ...
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_call_api"):
                    ...
                    if API_KEY:
                        ...
                        try:
                            # 1. 세션 생성 및 타임아웃 20초로 연장
                            with requests.Session() as s:
                                # 2. verify=False 옵션은 가급적 피하되, SSL 에러 발생 시 최후의 수단으로 고려
                                res = s.get(url, params=params, headers=headers, timeout=20)
                                
                                if res.status_code == 200:
                                    # 유니패스는 가끔 200을 주면서 본문에 에러 메시지를 담기도 합니다.
                            ...
                            # 구체적인 에러 확인을 위해 잠시 e를 찍어볼 수 있습니다.
                ...
                if st.button("적용", key="v5_hbtn_final", use_container_width=True):
                    if hs_in:
                        ...
                        try:
                            ...
                            st.session_state.calc_d = 8.0 # 에러 발생 시에도 8% 유지
                            st.error(f"오류")
