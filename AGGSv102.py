    ...
    if not sub_hs_df.empty:
        ...
        if not related_hsk.empty:
            for _, row in related_hsk.iterrows():
                code_val = row['HS10']
                # 여기서 국문과 영문을 합칩니다.
                k_name = str(row['name_kr']).strip()
                e_name = str(row['name_en']).strip()
                # AI에게 줄 재료 포맷: "CODE:8470103000 | DESC:전자계산기 [Electronic calculators]"
                hsk_list_text += f"CODE:{code_val} | DESC:{k_name} [{e_name}]\n"
        else:
            hsk_list_text = "해당 호의 상세 데이터가 없습니다."
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 지침:
                        ...
                        3) [검증 단계]: 만약 위 숫자 6자리(예: 847010)로 시작하는 데이터가 [참조용 HSK 리스트 데이터]에 단 하나도 없다면, 절대로 번호를 임의로 만들어내지 마세요. 대신 "제공된 데이터 내에 해당 6단위의 상세 HSK 정보가 없습니다."라고만 출력하세요.
