...
def load_aggs_master_data():
    ...
    def clean_hs_data(df):
        if 'HS_code' in df.columns:
            # 숫자를 문자열로 변환 -> 소수점 제거 -> 10자리로 맞춤 (앞에 0 채우기)
            df['HS_code'] = df['HS_code'].astype(str).str.split('.').str[0].str.zfill(10)
        return df

    df_master = clean_hs_data(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1FuqaAf2qr6wg76xF4ieWbDy4_ot5C2ClubYq33aMxGo/edit?usp=sharing"))
    df_tariff = clean_hs_data(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1kw9WMhHOkjE-GIqkuC97QQDB9GCQZdHwloPAOx_d_3E/edit?usp=sharing"))
    
    # 요건 데이터들 (동일하게 처리)
    df_req_im = clean_hs_data(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1kEMJujmsUVemtfO2oh1DoO9L_aGZMygtbVPrafiEPxQ/edit?usp=sharing", worksheet="Req_IM"))
    df_req_ex = clean_hs_data(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1kEMJujmsUVemtfO2oh1DoO9L_aGZMygtbVPrafiEPxQ/edit?usp=sharing", worksheet="Req_EX"))
    
    # 통합공고는 4~6자리이므로 zfill을 하지 않고 소수점만 제거
    def clean_cpan(df):
        df['HS_code'] = df['HS_code'].astype(str).str.split('.').str[0]
        return df
        
    df_cpan_im = clean_cpan(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1kEMJujmsUVemtfO2oh1DoO9L_aGZMygtbVPrafiEPxQ/edit?usp=sharing", worksheet="CPAN_IM"))
    df_cpan_ex = clean_cpan(conn.read(spreadsheet="https://docs.google.com/spreadsheets/d/1kEMJujmsUVemtfO2oh1DoO9L_aGZMygtbVPrafiEPxQ/edit?usp=sharing", worksheet="CPAN_EX"))
    
    return df_master, df_tariff, df_req_im, df_req_ex, df_cpan_im, df_cpan_ex
...
with tabs[1]:
    st.markdown("<div class='custom-header'>📘 HS 통합 정보 조회 (AGGS)</div>", unsafe_allow_html=True)
    
    # 1. 데이터 로드
    with st.spinner("마스터 데이터를 동기화 중입니다..."):
        df_m, df_t, df_ri, df_re, df_ci, df_ce = load_aggs_master_data()

    # 2. 검색창 레이아웃
    col_s1, col_s2 = st.columns([3, 1])
    with col_s1:
        target_hs = st.text_input("HS Code 10자리를 입력하세요 (예: 0101211000)", key="hs_input_main").strip().replace("-", "")
    
    if target_hs:
        # A. 기본 정보 (Master)
        master_res = df_m[df_m['HS_code'].astype(str).str.contains(target_hs)]
        
        if not master_res.empty:
            m = master_res.iloc[0]
            st.markdown(f"### 📦 {target_hs} | {m['Name_kor']}")
            
            # 레이아웃 구성 (기존 AFTUI 양식 참조)
            c1, c2 = st.columns(2)
            with c1:
                st.info(f"**영문품명:** {m['Name_Eng']}\n\n**수량단위:** {m['Qty_unit_code']} | **중량단위:** {m['Wt_unit_code']}")
            with c2:
                st.success(f"**성격분류:** {m['Chrctr_class_code_name']}\n\n**표준품명:** {m.get('Stndrd_name_kr', '정보없음')}")

            # B. 세율 정보 (Tariff)
            st.markdown("#### 💰 적용 세율 정보")
            t_res = df_t[df_t['HS_code'].astype(str).str.contains(target_hs)]
            if not t_res.empty:
                st.dataframe(t_res[['Trff_rate_class', 'Trff_rate', 'TrffpUnit']], use_container_width=True, hide_index=True)
            else:
                st.warning("등록된 세율 정보가 없습니다.")

            # C. 수입 요건 (세관장확인 + 통합공고)
            st.markdown("#### 📥 수입 관련 법령 및 요건")
            # 세관장확인 (10자리 일치)
            ri_res = df_ri[df_ri['HS_code'].astype(str).str.contains(target_hs)]
            # 통합공고 (앞자리 일치 로직 적용)
            ci_res = df_ci[df_ci.apply(lambda x: target_hs.startswith(str(x['HS_code'])), axis=1)]
            
            all_im = pd.concat([ri_res, ci_res])
            if not all_im.empty:
                st.table(all_im[['Law_code_name', 'apprv_agncy_code_name', 'Req_doc_name']])
            else:
                st.write("해당하는 수입 요건이 없습니다.")

            # D. 수출 요건 (세관장확인 + 통합공고)
            st.markdown("#### 📤 수출 관련 법령 및 요건")
            re_res = df_re[df_re['HS_code'].astype(str).str.contains(target_hs)]
            ce_res = df_ce[df_ce.apply(lambda x: target_hs.startswith(str(x['HS_code'])), axis=1)]
            
            all_ex = pd.concat([re_res, ce_res])
            if not all_ex.empty:
                st.table(all_ex[['Law_code_name', 'apprv_agncy_code_name', 'Req_doc_name']])
            else:
                st.write("해당하는 수출 요건이 없습니다.")
        else:
            st.error("입력하신 HS Code를 찾을 수 없습니다. 정확한 10자리를 입력해 주세요.")
