...
st.set_page_config(page_title="A.G.G.S - 전문 관세 행정 서비스", layout="wide")
...
# ★변경내역★시작★ [논리적 차단 및 NameError 방지 통합본]
...
with tabs[0]:
        ...
        if not uploaded_file and not input_summary:
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    # [2-2] 제미나이 멀티모달 분석 (발췌 지식 및 인용 지침 강제 주입: 변경내역★★★★)
                    model = genai.GenerativeModel('gemini-2.0-flash')
    ...
    if st.session_state.ai_report_done:
        ...
        st.divider()
        st.markdown("#### 📧 10단위 HSK 및 추가 검토가 필요하신 경우, 전문 관세사에게 의뢰하세요.")
...
with tabs[1]:
    # CSS 스타일 적용 (가운데 정렬, 숫자 우측 정렬 등)
    st.markdown(f"""
        <style>
            .stDataFrame div[data-testid="stTable"] {{ font-size: {C_FONT}; }}
            .title-text {{ font-size: {T_FONT}; font-weight: bold; color: #1E3A8A; margin-bottom: 10px; }}
            .rate-val {{ font-size: {RATE_FONT}; font-weight: bold; color: #D32F2F; }}
            .label-text {{ font-size: {C_FONT}; color: #64748B; }}
            .stAlert {{ background-color: #F0FDF4; border: 1px solid #BBF7D0; color: #166534; }}
        </style>
    """, unsafe_allow_html=True)
    ...
    # 검색창
    target_hs = st.text_input("HS Code 10자리 입력", key="hs_main_search").strip().replace("-", "").zfill(10)

    if target_hs != "0000000000":
        # --- 1. 마스터 타이틀 ---
        st.markdown(f"<div class='title-text'>🔍 {target_hs} 조회 결과</div>", unsafe_allow_html=True)

        # --- 2. 기본/표준 품명 섹션 ---
        col_m1, col_m2 = st.columns(2)
        with col_m1:
            st.write("📂 **표준품명**")
            sn_val = df_m_sn[df_m_sn['HS_code'] == target_hs]['Item_name'].iloc[0] if not df_m_sn[df_m_sn['HS_code'] == target_hs].empty else "정보 없음"
            st.markdown(f"<div style='background-color: #F0FDF4; padding: 15px; border-radius: 10px; border-left: 5px solid #22C55E; color: #166534; font-size: {C_FONT};'>{sn_val}</div>", unsafe_allow_html=True)
        with col_m2:
            st.write("📂 **기본품명**")
            m_row = df_m_hs[df_m_hs['HS_code'] == target_hs]
            if not m_row.empty:
                st.info(f"**국문:** {m_row.iloc[0]['Name_kor']}\n\n**영문:** {m_row.iloc[0]['Name_Eng']}")
            else: st.info("정보 없음")
        st.divider()

        # --- 3. 관세율 정보 섹션 ---
        st.markdown("<div class='title-text'>💰 관세율 정보</div>", unsafe_allow_html=True)
        
        # A세율, C세율 추출
        ac_data = df_t_ac[df_t_ac['HS_code'] == target_hs]
        ...
        rate_config = {
            "코드": st.column_config.TextColumn("코드", help="세율 구분 코드", alignment="center"),
            "세율명칭": st.column_config.TextColumn("세율명칭", alignment="center"),
            "세율": st.column_config.NumberColumn("세율", format="%.1f%%", alignment="right")
        }

        with col_r1:
            st.markdown(f"<span class='label-text'>기본세율(A)</span><br><span class='rate-val'>{val_a}%</span>", unsafe_allow_html=True)
            st.write("**기타세율**")
            etc_res = df_t_etc[df_t_etc['HS_code'] == target_hs][['Trff_rate_class', 'Trff_rate']]
            etc_res = etc_res.merge(df_m_tc[['Trff_rate_class', 'Trff_rate_class_name_kr']], on='Trff_rate_class', how='left')
            etc_res.columns = ['코드', '세율', '세율명칭']
            st.dataframe(etc_res[['코드', '세율명칭', '세율']], use_container_width=True, hide_index=True)
            
        with col_r2:
            st.markdown(f"<span class='label-text'>WTO협정세율(C)</span><br><span class='rate-val'>{val_c}%</span>", unsafe_allow_html=True)
            st.write("**협정세율(FTA)**")
            fta_all = pd.concat([df_t_f1, df_t_f2, df_t_fm])
            fta_res = fta_all[fta_all['HS_code'] == target_hs][['Trff_rate_class', 'Trff_rate']]
            fta_res = fta_res.merge(df_m_tc[['Trff_rate_class', 'Trff_rate_class_name_kr']], on='Trff_rate_class', how='left')
            fta_res.columns = ['코드', '세율', '세율명칭']
            st.dataframe(fta_res[['코드', '세율명칭', '세율']], height=300, use_container_width=True, hide_index=True)

        st.divider()

        # --- 4. 세관장확인대상 (수출입요건) ---
        st.markdown("<div class='title-text'>🛡️ 세관장확인대상 (수출입요건)</div>", unsafe_allow_html=True)
        col_req1, col_req2 = st.columns(2)
        
        req_config = {
            "법령명": st.column_config.TextColumn("법령명", alignment="center"),
            "승인기관": st.column_config.TextColumn("승인기관", alignment="center"),
            "서류명": st.column_config.TextColumn("서류명", alignment="left")
        }

        with col_req1:
            st.write("📥 **수입요건**")
            im_req = df_r_im[df_r_im['HS_code'] == target_hs][['Law_code_name', 'apprv_agncy_code_name', 'Req_doc_name']]
            im_req.columns = ['법령명', '승인기관', '서류명']
            st.dataframe(im_req.sort_values('법령명'), use_container_width=True, hide_index=True)
            
        with col_req2:
            st.write("📤 **수출요건**")
            ex_req = df_r_ex[df_r_ex['HS_code'] == target_hs][['Law_code_name', 'apprv_agncy_code_name', 'Req_doc_name']]
            ex_req.columns = ['법령명', '승인기관', '서류명']
            st.dataframe(ex_req.sort_values('법령명'), use_container_width=True, hide_index=True)

        # --- 5. 통합공고 ---
        st.markdown("<div class='title-text'>🔰 통합공고</div>", unsafe_allow_html=True)
        col_cp1, col_cp2 = st.columns(2)

        def format_guide(text):
            return f'<div style="font-size: 10px; line-height: 1.2;">{text}</div>'
        
        cp_config = {
            "법령명": st.column_config.TextColumn("법령명", alignment="center"),
            "대상": st.column_config.TextColumn("대상", alignment="center"),
            "요령": st.column_config.TextColumn("요령", alignment="left") # 텍스트 컬럼으로 유지
        }

        with col_cp1:
            st.write("📥 **수입요령**")
            df_c_im_clean = df_c_im.dropna(subset=['HS_code'])
            cp_im_res = df_c_im_clean[df_c_im_clean.apply(lambda x: str(target_hs).startswith(str(x['HS_code']).split('.')[0]), axis=1)]
            if not cp_im_res.empty:
                cp_im_display = cp_im_res[['Law_name', 'Name', 'Guide']].copy()
                cp_im_display.columns = ['법령명', '대상', '요령']
                st.dataframe(cp_im_display.sort_values('법령명'), use_container_width=True, hide_index=True)
            else: st.write("조회된 수입요령 정보가 없습니다.")

        with col_cp2:
            st.write("📤 **수출요령**")
            df_c_ex_clean = df_c_ex.dropna(subset=['HS_code'])
            cp_ex_res = df_c_ex_clean[df_c_ex_clean.apply(lambda x: str(target_hs).startswith(str(x['HS_code']).split('.')[0]), axis=1)]
            if not cp_ex_res.empty:
                cp_ex_display = cp_ex_res[['Law_name', 'Name', 'Guide']].copy()
                cp_ex_display.columns = ['법령명', '대상', '요령']
                st.dataframe(cp_ex_display.sort_values('법령명'), use_container_width=True, hide_index=True)
            else: st.write("조회된 수출요령 정보가 없습니다.")
...
with tabs[3]:
    ...
    # --- 2. 우편물(EMS) 조회 로직 (Retry 로직 및 세션 강화) ---
    if post_btn:
    ...
    # [3-2] 우편물 출력 (항목 최적화 및 헤더 고정/정렬 반영) ★
    if st.session_state.last_search_type == "POST" and st.session_state.post_result:
        ...
        if items:
            ...
            # 테이블 생성 (관리번호를 가장 앞으로, 진행단계/장소는 제외)
...
# --- [Tab 5] 세액계산기 (동작 확인된 코드 기반 안정화 버전) ---
with tabs[4]:
    st.markdown("""
        <style>
    ...
    with st.container(border=True):
            ...
            with l_r2[2]:
                if st.button("환율조회", use_container_width=True, key="v5_call_api"):
                    ...
                    if API_KEY:
                        ...
                        # [안정화 장치 1, 2] 세션 및 헤더 보강 ★
                        headers = {"User-Agent": "Mozilla/5.0", "Connection": "close"}
                        session = requests.Session()
                        
                        try:
                            # [안정화 장치 3] 타임아웃 넉넉히 설정 ★
        ...
        with c_right:
            ...
            with h2:
                if st.button("적용", key="v5_hbtn_final", use_container_width=True):
                    if hs_in:
                        ...
                        conn = sqlite3.sqlite3.connect("customs_master.db")
            ...
            with r_r2_1: a_d = st.number_input("관세율 (%)", value=st.session_state.calc_d, key="v5_duty_final")
            with r_r2_2: a_v = st.number_input("부가세율 (%)", value=10.0, key="v5_vat_final")

            # 실시간 과세표준 계산 (p_ex 참조 유지)
...
# --- [Tab 6] 관리자 모드 (데이터 업로드 완벽 복구 + 신규 관리 기능 통합) ---
if len(tabs) > 5:
                            ...
                            if curr_hash == db_pw:
                                if new_pw == new_pw_chk:
                                    if len(new_pw) < 4:
                                        ...
                                        # [핵심 수정] 새로운 열을 만들지 않고 기존 컬럼(Password)에 덮어쓰기
                                        df_users.loc[mask, pw_col] = new_hash
                                        
                                        try:
                                            # 4. 구글 시트 업데이트 (전체 데이터프레임 전달)
                                            conn_gs.update(
            ...
            if st.session_state.admin_verified:
                ...
                with adm_tabs[1]:
                    ...
                    if not df_users.empty:
                        ...
                        if cr2.button("🗑️ 해당 고객사 계정 삭제", type="primary", use_container_width=True):
                ...
                with adm_tabs[2]:
                    ...
                    try:
                        ...
                        if not df_log_view.empty:
                            # [요청사항 반영] 1. 최신 이력이 가장 상단으로 오도록 역순 정렬
                            df_log_view = df_log_view.iloc[::-1]
                            
                            # [요청사항 반영] 2. 기본 5줄 높이(약 200px), 스크롤 가능하도록 출력
                            st.dataframe(
                                df_log_view,
                                use_container_width=True,
                                hide_index=True,
...
with f_left:
    # 모바일 대응을 위해 왼쪽 구역을 다시 분할 (연락처 : 버튼들)
    # PC에서는 한 줄, 모바일에서는 적절히 배치됨
    fl_col1, fl_col2 = st.columns([0.5, 0.5])
    
    with fl_col1:
        st.markdown(f"<div style='padding-top: 5px; font-size: {CONTENT_FONT_SIZE};'><b>📞 010-8859-0403 (이지스)</b></div>", unsafe_allow_html=True)
    
    with fl_col2:
        # 링크 버튼들을 나란히 배치 (작은 열로 쪼개어 모바일 대응)
...
with f_right:
    # 로그인이 되어 있을 때만 우측 끝에 표시
    if st.session_state.get('logged_in', False):
        u_col1, u_col2 = st.columns([0.6, 0.4], gap="small")
        
        with u_col1:
            # 유저 태그 높이를 로그아웃 버튼과 시각적으로 맞춤 (40px -> 34px 조정)
            st.markdown(f"""
        ...
        with u_col2:
            # Logout 버튼 (전체 너비 사용으로 모바일 터치 편의성 증대)
