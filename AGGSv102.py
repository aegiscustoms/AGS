...
# --- [Tab 2] HS정보
with tabs[1]:
    # --- 1. 스타일 설정 (Header 정렬 및 10px 폰트 강제 적용) ---
    st.markdown("""
        <style>
            /* [수정] 모든 데이터프레임의 열 제목(Header)을 강제로 가운데 정렬 */
            /* Streamlit의 다양한 헤더 구조를 모두 타겟팅합니다. */
            div[data-testid="stDataFrame"] div[role="columnheader"] p,
            div[data-testid="stDataFrame"] th div,
            div[data-testid="stDataFrame"] [data-testid="stTableITitle"] {
            ...
            /* [수정] 통합공고 요령 전용 폰트 (10px) 및 좌측 정렬 */
            /* 특정 클래스로 감싸진 테이블의 데이터 셀(td)만 조절 */
            .small-font-table [data-testid="stDataFrame"] [role="gridcell"] {
            ...
            .box-blue { background-color: #EFF6FF; border-color: #2563EB; color: #1E40AF; font-size: 16px; }
        </style>
    """, unsafe_allow_html=True)
    ...
    target_hs_raw = st.text_input("HS Code 10자리 입력", key="hs_final_search_v4").strip().replace("-", "").replace(".", "").zfill(10)
    ...
    if target_hs_raw != "0000000000":
        ...
        with col_cp1:
            ...
            if not cp_im_res.empty:
                # [중요] 반드시 'small-font-table' 클래스를 가진 div로 감싸야 10px가 적용됩니다.
