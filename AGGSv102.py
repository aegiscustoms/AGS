...
def load_hs_resources():
    ...
    try:
        ...
        # 컬럼명을 코드에서 사용하기 편하게 영어/공통명칭으로 변경
...
def get_legal_ground(p_name, manual_txt, hs_df, sub_hs_df): 
    ...
    if not sub_hs_df.empty:
        # 1. 비교 대상을 순수 숫자로 정제 (공백 제거)
        search_target = str(h_code).strip() 
        
        # 2. 데이터프레임의 모든 HS10 번호를 문자열로 통일
        sub_hs_df['HS10'] = sub_hs_df['HS10'].astype(str).str.replace(".0", "").str.strip()
        
        # 3. 매칭 실행
        related_hsk = sub_hs_df[sub_hs_df['HS10'].str.startswith(search_target, na=False)]
        
        if not related_hsk.empty:
            for _, row in related_hsk.head(20).iterrows():
                raw_hsk = row['HS10']
                # 4. 출력할 때만 관세사님이 원하시는 포맷으로 가공
                if len(raw_hsk) >= 10:
                    f_hsk = f"{raw_hsk[:4]}.{raw_hsk[4:6]}-{raw_hsk[6:10]}"
                else:
                    f_hsk = raw_hsk # 10자리가 안 될 경우 대비
                
                p_name_en = row['품명_en'] if pd.notna(row['품명_en']) else "No Description"
                hsk_list_text += f"- {f_hsk} [{p_name_en}]\n"
            ...
            with st.spinner("🔍 AI 전문 관세사가 물품정보를 분석 중입니다..."):
                try:
                    ...
                    - "4. 추천 HS Code내 분류가능한 HSK" 항목 작성 지침:
                        1) 먼저 "2. 추천 HS Code (6자리)"에서 최적의 번호를 확정하세요.
                        2) 확정된 6단위 번호(앞 6자리)와 [참조용 HSK 리스트 데이터] 내의 번호들을 대조하세요.
                        3) 6단위가 일치하는 모든 10단위 HSK를 데이터에서 찾아 나열하세요.
                        4) 출력 형식은 반드시 'XXXX.XX-XXXX 소호의 용어 [영문 품명]' 형태를 유지하세요.
                        5) 만약 데이터 내에 일치하는 6단위가 없다면, 임의로 생성하지 말고 "제공된 데이터 내에 해당 6단위의 상세 HSK를 확인할 수 없습니다."라고 명시하세요.
                    ...
                    **4. 추천 HS Code내 분류가능한 HSK** 
                       - (여기에 2번에서 추천된 6단위와 일치하는 [참조용 HSK 리스트 데이터] 내의 모든 10단위 리스트를 나열하세요.)                       
