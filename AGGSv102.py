...
with tabs[1]:
    ...
    if target_hs_raw != "0000000000":
        ...
        st.markdown(f"""
            <style>
                /* 1. 모든 데이터프레임의 열 제목(Header) 가운데 정렬 */
        ...
        with col_cp2:
            ...
            if not cp_ex_res.empty:
                ...
                st.markdown("<div class='small-font-table'>", unsafe_allow_html=True)
                st.dataframe(cp_ex_display.sort_values('법령명'), height=250, use_container_width=True, hide_index=True, column_config=cp_config)
                st.markdown("</div>", unsafe_allow_html=True)
            else: st.write("조회된 정보 없음")
