...
def load_hs_resources():
    ...
    # 2. 10단위 소호 리스트 (subHeadings) 로드
    try:
        # 파일명과 경로를 말씀하신 대로 지정
        df_sub_hs = pd.read_csv('knowledge_base/headings/HS_subHeadings_All.csv', dtype={'HS10': str, '번호': str})
    except Exception as e:
...
def get_legal_ground(p_name, manual_txt, hs_df, sub_hs_df): 
    ...
    # [추가] 해당 4단위로 시작하는 10단위 HSK 리스트 추출
    # CSV의 컬럼명이 'HS10'이고, '번호'가 4단위라고 가정할 때:
    related_hsk_df = sub_hs_df[sub_hs_df['HS10'].str.startswith(h_code, na=False)]
    hsk_list_text = ""
    if not related_hsk_df.empty:
        # 최대 15개까지만 나열 (프롬프트 길이 조절)
        for _, row in related_hsk_df.head(15).iterrows():
            hsk_list_text += f"- {row['HS10']}: {row['품명']}\n"
