...
with tabs[1]:
    ...
    # --- 1. 검색창 및 HS 코드 변환 (0000.00-0000 형식) ---
    target_hs_raw = st.text_input("HS Code 10자리 입력", key="hs_final_search_v3").strip().replace("-", "").replace(".", "").zfill(10)
    
    # 출력용 포맷팅: 1234.56-7890 형식
    formatted_hs = f"{target_hs_raw[:4]}.{target_hs_raw[4:6]}-{target_hs_raw[6:]}"

    if target_hs_raw != "0000000000":
        # 개별 항목별 스타일 정의 (전역 변수 제거)
        st.markdown(f"""
            <style>
                /* 타이틀 스타일 */
                .res-title {{ font-size: 18px; font-weight: bold; color: #1E3A8A; margin-bottom: 15px; }}
                
                /* 1. 표준품명/기본품명 컨테이너 (디자인 통일 및 테두리 반영) */
                .box-base {{
                    padding: 15px; border-radius: 8px; border-left: 6px solid; min-height: 110px; line-height: 1.6; margin-bottom: 10px;
                }}
                /* 표준품명: 초록계열, 폰트 16px 강제적용 */
                .box-green {{ background-color: #F0FDF4; border-color: #16A34A; color: #166534; font-size: 16px !important; font-weight: 600; }}
                /* 기본품명: 파란계열, 사이드 테두리 추가 */
                .box-blue {{ background-color: #EFF6FF; border-color: #2563EB; color: #1E40AF; font-size: 13px; }}
                
                /* 5. 통합공고 요령 폰트 10px 강제 적용 (데이터프레임 내부) */
                div[data-testid="stDataFrame"] td {{ font-size: 12px; }} /* 기본값 */
                .small-font-table div[data-testid="stDataFrame"] td {{ font-size: 10px !important; }}
            </style>
        """, unsafe_allow_html=True)
        ...
        # 표 설정: 모든 열제목 가운데 정렬 및 값 정렬
        rate_config = {
            "코드": st.column_config.TextColumn("코드", alignment="center"),
            "세율명칭": st.column_config.TextColumn("세율명칭", alignment="center"),
            "세율": st.column_config.NumberColumn("세율", format="%.1f%%", alignment="center") # 관세사님 요청: 가운데 정렬
        }

        with col_r1:
            st.markdown(f"기본세율(A) <b style='font-size:22px; color:#D32F2F;'>{val_a}%</b>", unsafe_allow_html=True)
            st.write("**기타세율**")
        ...
        with col_r2:
            st.markdown(f"WTO협정세율(C) <b style='font-size:22px; color:#D32F2F;'>{val_c}%</b>", unsafe_allow_html=True)
            st.write("**협정세율(FTA)**")
        ...
        req_config = {
            ...
            "서류명": st.column_config.TextColumn("서류명", alignment="center") # 모두 가운데 정렬
        }
        ...
        with col_req1:
            ...
            st.dataframe(im_req.sort_values('법령명'), use_container_width=True, hide_index=True, column_config=req_config)
            
        with col_req2:
            st.write("📤 **수출요건**")
            ex_req = df_r_ex[df_r_ex['HS_code'] == target_hs_raw][['Law_code_name', 'apprv_agncy_code_name', 'Req_doc_name']]
            ex_req.columns = ['법령명', '승인기관', '서류명']
            st.dataframe(ex_req.sort_values('법령명'), use_container_width=True, hide_index=True, column_config=req_config)

        # --- [수정 5] 통합공고 (요령 10px 적용) ---
        ...
        cp_config = {
            ...
            "요령": st.column_config.TextColumn("요령", alignment="left") # 요령은 좌측 정렬이 가독성이 좋음
        }
