...
def init_db():
    ...
    # 통계부호 테이블
    c.execute("CREATE TABLE IF NOT EXISTS stat_codes (category TEXT, code TEXT, name TEXT)")
    conn.commit()
    conn.close()
...
STAT_LIST = sorted([
    ...
    "내국세구분코드", "내국세세종코드", "농특세과세구분코드", "담당자변경사유코드", "담보업체변경사유코드", "담보업체취소사유코드",
    "담보제공사유코드", "담보종류코드", "대륙종류코드", "대상업체구분코드", "동향구분코드", "동향유형코드", "마약물품종류코드",
    "마약약리작용구분코드", "마약은닉장소코드", "마약의약용도구분코드", "마약적발경위구분코드", "마약적발관련자처리상태코드",
    "마약적발외부기관코드", "마약조직원역할코드", "마약종류코드", "마약코드", "마약투여방법코드", "마약형태코드", "말소구분코드",
    "물품반입구분코드", "물품용도코드", "물품용역업체항공영업종류코드", "물품용역업체해상영업종류코드", "물품폐기사유코드",
    "미가산사유코드", "밀수근원코드", "밀수신고접수방법코드", "반입경로코드", "반입유형코드", "반출기간연장구분코드",
    "반출사유코드", "반출유형코드", "반출입외화용도구분코드", "발각원인코드", "범칙물품상표코드", "범칙물품유형코드",
    "범칙상세경로코드", "범칙수단코드", "범행동기코드", "법령종류코드", "법령코드", "법무부출입국도시코드", "법조문구분코드",
    "병역구분코드", "보세구역구분코드", "보세구역반출입정정항목코드", "보세구역반출입화물종류코드", "보세구역부호",
    "보세운송검사구분코드", "보세운송검사대상구분코드", "보세운송검사지정상태코드", "보세운송검사처리상태코드",
    "보세운송승인신청담보구분코드", "보세운송승인신청사유코드", "보수작업형태코드", "보정심사생략구분코드",
    "보정심사수작업선별사유코드", "보정심사처리상태코드", "봉인내역코드", "봉인지정내역처리상태코드", "부가세과세구분코드",
    "분석기관구분코드", "분할반출입구분코드", "분할통합사유코드", "비위유형코드", "사건근거구분코드", "사업자구분코드",
    "사업종류코드", "사이트유형코드", "사전세액심사선별기준코드", "사후관리방법코드", "사후관리비대상사유코드",
...
with tabs[1]:    
    st.markdown(f"""
        <style>
            .custom-text {{ font-size: 15px !important; line-height: 1.6; }}
            .custom-header {{ font-size: 15px !important; font-weight: bold; color: #1E3A8A; margin-bottom: 8px; }}
            .custom-value {{ font-size: 12px !important; font-weight: 500; background-color: #F8FAFC; padding: 4px; border-radius: 4px; }}
            .stDataFrame div[data-testid="stTable"] {{ font-size: 12px !important; }}
        </style>
    """, unsafe_allow_html=True)

    target_hs = st.text_input("조회할 HSK 10자리를 입력하세요 (0 포함)", key="hs_info_input", placeholder="예: 0101211000")
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            if not m.empty:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("<div class='custom-text'><b>[기본 품명]</b></div>", unsafe_allow_html=True)
                ...
                if not r.empty:
                    ...
                    st.dataframe(rf.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
                    
                    st.markdown("<div class='custom-text'><b>기타세율</b></div>", unsafe_allow_html=True)
                    st.dataframe(re_etc.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
                else: st.warning("등록된 세율 데이터가 없습니다.")

                st.divider()
                col_req1, col_req2 = st.columns(2)
                with col_req1:
                    st.markdown("<div class='custom-header'>🛡️ 세관장확인대상 (수입)</div>", unsafe_allow_html=True)
                    if not req_i.empty: st.table(req_i)
                    else: st.success("대상 아님")
                with col_req2:
                    st.markdown("<div class='custom-header'>🛡️ 세관장확인대상 (수출)</div>", unsafe_allow_html=True)
                    if not req_e.empty: st.table(req_e)
                    else: st.success("대상 아님")
            else:
                st.error("해당 HS코드 정보가 DB에 없습니다. (10자리 입력을 확인해주세요)")

# --- [Tab 3] 통계부호 (자동완성 선택 및 명칭 기반 코드 검색) ---
with tabs[2]:
    st.subheader("📊 통계부호 통합 검색")
    
    # 1) 드롭다운 대신 검색어 입력을 통한 카테고리 선택 (selectbox의 index 활용)
    # 수백 개의 목록에서 타이핑으로 검색하여 선택할 수 있습니다.
    target_cat = st.selectbox(
        "검색할 통계부호 카테고리를 입력/선택하세요", 
        STAT_LIST, 
        index=0,
        help="검색하고자 하는 부호 명칭을 입력하면 목록이 필터링됩니다."
    )
    
    # 2) 검색창 명칭 수정 및 한글내역(명칭) 기반 검색 로직
    search_kw = st.text_input(
        "키워드 검색", 
        placeholder="검색할 한글 명칭(내역)을 입력하세요",
        help="입력하신 키워드가 포함된 명칭을 찾아 해당 코드를 보여줍니다."
    )
    
    if st.button("부호 조회", use_container_width=True):
        if search_kw:
            conn = sqlite3.connect("customs_master.db")
            # 명칭(name)에서 키워드를 검색하고 코드와 명칭을 출력합니다.
            query = """
                SELECT code as '통계코드', name as '한글내역' 
                FROM stat_codes 
                WHERE category = ? AND name LIKE ?
            """
            res = pd.read_sql(query, conn, params=(target_cat, f"%{search_kw}%"))
            conn.close()
            
            if not res.empty:
                st.markdown(f"**🔍 '{target_cat}' 내 '{search_kw}' 검색 결과**")
                # 가운데 정렬 및 폰트 크기 적용
                st.dataframe(
                ...
                st.warning(f"'{target_cat}' 카테고리에 '{search_kw}'를 포함하는 명칭이 없습니다.")
        else:
            st.info("검색할 키워드를 입력해 주세요.")

# --- [Tab 4, 5] 화물정보/계산기 ---
with tabs[3]: st.info("📦 화물통관진행정보 준비 중")
with tabs[4]: st.info("🧮 세액계산기 준비 중")

# --- [Tab 6] 관리자 (전면 수정 및 일괄 업로드 로직 완성) ---
if st.session_state.is_admin:
    with tabs[-1]:
        st.header("⚙️ 관리자 데이터 센터")
        
        # 구역 1: HS 마스터
        st.subheader("📁 1. HS 및 관세 마스터 관리")
        m_list = ["HS코드(마스터)", "표준품명", "관세율", "관세율구분", "세관장확인(수입)", "세관장확인(수출)"]
        cols = st.columns(3)
        for i, m_name in enumerate(m_list):
            with cols[i%3]:
                st.write(f"**{m_name}**")
                up = st.file_uploader(f"{m_name} 파일", type="csv", key=f"admin_{m_name}", label_visibility="collapsed")
                if up:
                    if st.button(f"반영", key=f"btn_{m_name}"):
                        try:
                            ...
                            elif m_name == "세관장확인(수입)":
                                # HS부호, 신고인확인법령코드명, 요건승인기관코드명, 요건확인서류명
                                df_map = df[['HS부호', '신고인확인법령코드명', '요건승인기관코드명', '요건확인서류명']].copy()
                                df_map.columns = ['hs_code', 'law', 'agency', 'document']
                                df_map['hs_code'] = df_map['hs_code'].astype(str).str.replace(r'[^0-9]', '', regex=True).str.zfill(10)
                                df_map.to_sql('req_import', conn, if_exists='replace', index=False)
                                
                            elif m_name == "세관장확인(수출)":
                                df_map = df[['HS부호', '신고인확인법령코드명', '요건승인기관코드명', '요건확인서류명']].copy()
                                df_map.columns = ['hs_code', 'law', 'agency', 'document']
                                df_map['hs_code'] = df_map['hs_code'].astype(str).str.replace(r'[^0-9]', '', regex=True).str.zfill(10)
                                df_map.to_sql('req_export', conn, if_exists='replace', index=False)
        ...
        st.info("💡 여러 개의 통계부호 CSV 파일을 한 번에 드래그하세요. (파일명 = 부호명칭)")
        multi_files = st.file_uploader("통계부호 파일 드래그 & 드롭", type="csv", accept_multiple_files=True)
        
        if multi_files:
            if st.button("🚀 선택한 모든 파일 일괄 업로드 실행", use_container_width=True):
                conn = sqlite3.connect("customs_master.db")
                success_list = []
                for f in multi_files:
                    ...
                    if cat in STAT_LIST:
                        try:
                            ...
                            success_list.append(cat)
                        except Exception as e:
                            st.error(f"{cat} 파일 처리 중 오류: {e}")
                conn.commit()
                conn.close()
                st.success(f"업로드 완료: {', '.join(success_list)}")
