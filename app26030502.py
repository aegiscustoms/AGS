...
# --- [Tab 2] HS정보 (레이아웃 최적화 및 우선순위 정렬 반영) ---
with tabs[1]:
    # CSS 스타일 정의 (12px 최적화 및 박스 디자인)
    st.markdown("""
        <style>
            .custom-header { font-size: 14px !important; font-weight: bold; color: #1E3A8A; margin-bottom: 8px; border-left: 4px solid #1E3A8A; padding-left: 8px; }
            .custom-title { font-size: 13px !important; font-weight: bold; color: #334155; margin-bottom: 4px; }
            .custom-value { font-size: 12px !important; line-height: 1.5; background-color: #F8FAFC; padding: 8px; border-radius: 4px; border: 1px solid #E2E8F0; margin-bottom: 12px; min-height: 60px; }
            /* 데이터프레임 폰트 조절 */
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            # 1. 데이터 호출 (기존 누락된 표준영문명 std_name_en 추가)
            ...
            r_query = f"""
                ...
                FROM rates r LEFT JOIN rate_names n ON r.type = n.code
                WHERE r.hs_code = '{hsk}'
            """
            ...
            if not m.empty:
                ...
                # 1. 품명 섹션 (표준품명 좌측, 기본품명 우측)
                col_left, col_right = st.columns(2)
                with col_left:
                    st.markdown("<div class='custom-title'>표준품명</div>", unsafe_allow_html=True)
                    if not std.empty:
                        st.markdown(f"<div class='custom-value'><b>한글:</b> {std['std_name_kr'].values[0]}<br><b>영문:</b> {std['std_name_en'].values[0]}</div>", unsafe_allow_html=True)
                    else:
                        st.markdown("<div class='custom-value'>등록된 표준품명 정보가 없습니다.</div>", unsafe_allow_html=True)
                ...
                if not r_all.empty:
                    ...
                    # 카테고리 분류
                    rate_a = r_all[r_all['코드'] == 'A']
                    rate_c = r_all[r_all['코드'] == 'C']
                    rate_f = r_all[r_all['코드'].str.startswith('F', na=False)]
                    rate_etc = r_all[~r_all['코드'].isin(['A', 'C']) & ~r_all['코드'].str.startswith('F', na=False)]
                    
                    # (1) 상단 메트릭: 기본(A) 및 WTO(C)
                    m1, m2 = st.columns(2)
                    with m1: st.metric("기본세율 (A)", rate_a['세율'].values[0] if not rate_a.empty else "-")
                    with m2: st.metric("WTO협정세율 (C)", rate_c['세율'].values[0] if not rate_c.empty else "-")
                    
                    # (2) 기타세율 (농특세, 개소세 등)
                    st.markdown("<div class='custom-title' style='margin-top:10px;'>기타세율</div>", unsafe_allow_html=True)
                    if not rate_etc.empty:
                        st.dataframe(rate_etc.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
                    else: st.caption("해당 없음")

                    # (3) 협정세율 (FTA)
                    st.markdown("<div class='custom-title'>협정세율 (FTA)</div>", unsafe_allow_html=True)
                    if not rate_f.empty:
                        st.dataframe(rate_f.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
                    else: st.caption("해당 없음")
                ...
                with c_imp:
                    st.markdown("<div class='custom-title'>[수입 요건]</div>", unsafe_allow_html=True)
                    if not req_i.empty:
                        st.dataframe(req_i, hide_index=True, use_container_width=True)
                    else: st.success("수입 세관장확인대상 아님")
                with c_exp:
                    st.markdown("<div class='custom-title'>[수출 요건]</div>", unsafe_allow_html=True)
                    if not req_e.empty:
                        st.dataframe(req_e, hide_index=True, use_container_width=True)
                    else: st.success("수출 세관장확인대상 아님")
            else:
                st.error("해당 HS코드에 대한 마스터 정보가 존재하지 않습니다.")
