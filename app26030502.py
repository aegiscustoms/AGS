...
with tabs[2]:
    ...
    with col1:
        target_cat = st.selectbox("통계부호 카테고리", STAT_LIST)
    with col2:
        search_kw = st.text_input(f"{target_cat} 내 키워드 검색", placeholder="코드 또는 명칭 입력")
    
    if st.button("부호 조회"):
        conn = sqlite3.connect("customs_master.db")
        query = "SELECT code as '코드', name as '명칭' FROM stat_codes WHERE category = ? AND (code LIKE ? OR name LIKE ?)"
        res = pd.read_sql(query, conn, params=(target_cat, f"%{search_kw}%", f"%{search_kw}%"))
        conn.close()
        if not res.empty: st.dataframe(res.style.set_properties(**{'text-align': 'center'}), hide_index=True, use_container_width=True)
