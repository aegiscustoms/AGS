...
# --- [AFTUI26031007] 전역 디자인 설정 (엘박스 스타일) --- #변경내역★★
...
# --- Gemini API 및 모델 초기화 (에러 방지를 위해 상단 배치) --- #변경내역★★
api_key = st.secrets.get("GEMINI_KEY")
if api_key:
    genai.configure(api_key=api_key)
    model = genai.GenerativeModel('gemini-2.0-flash') # NameError: model 방지 #변경내역★★
else:
...
def display_hsk_details(hsk_code, prob=""):
    ...
    conn.close()
    if not master.empty: st.success(f"✅ [{code_clean}] {master['name_kr'].values[0]} {f'({prob})' if prob else ''}")

# --- [Tab 1] HS검색 (완벽 복구) --- #변경내역★★
with tabs[0]:
    ...
    if st.button("HS분석 실행", use_container_width=True):
        if u_img or u_input:
            with st.spinner("AI 관세사가 분석 중..."):
                try:
                    # 바이블(원본) 프롬프트 100% 복구 #변경내역★★
                    ...
                    # model 객체 사용 (상단에 정의됨) #변경내역★★
                    res = model.generate_content(content)
                    
                    st.markdown("### 📋 분석 리포트") #변경내역★★
                    st.write(res.text)
    ...
    with tabs[-1]:
        ...
        for i, m_name in enumerate(m_list):
            with cols[i%3]:
                ...
                if up and st.button("반영", key=f"btn_{m_name}"): #변경내역★★
                    ...
                    if df is not None:
                        ...
                        try:
                            ...
                            if m_name in t_map: df_map.to_sql(target_tbl[m_name] if 'target_tbl' in locals() else t_map[m_name], conn, if_exists='replace', index=False)
