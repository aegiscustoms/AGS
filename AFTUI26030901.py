...
# --- [AFTUI26031014] 전역 디자인 설정 (좌우 꽉 채우기 및 Sticky Header) --- #변경내역★★
...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        /* [탭2 전용] 테이블 레이아웃: 화면 꽉 채우기 및 항목명 고정 */ #변경내역★★
        .full-width-table {{ 
            width: 100% !important; 
            min-width: 100% !important; /* 화면 좌우 끝까지 확장 */
            border-collapse: collapse !important; 
            table-layout: fixed !important; 
            margin: 0 !important;
        }}
        
        /* 협정세율 스크롤 시 항목명 상단 고정 (Sticky Header) */ #변경내역★★
...
with tabs[3]:
    ...
    # 입력창: 세션 상태와 연동하여 자동 완성 지원 #변경내역★★
    with c_b: 
        bl_no = st.text_input("B/L 번호", value=st.session_state.bl_val, key="bl_search", help="B/L 입력 시 화물관리번호가 자동 완성됩니다.")
    with c_m: 
        mrn_no = st.text_input("화물관리번호", value=st.session_state.mrn_val, key="mrn_search", help="화물관리번호 입력 시 B/L 번호가 자동 완성됩니다.")
    
    if st.button("실시간 조회 실행", use_container_width=True):
        # API 키 유효성 검사
        API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
        if not API_KEY:
            st.error("❌ UNIPASS API 키가 설정되지 않았습니다.")
        else:
            with st.spinner("관세청 유니패스 통신 중..."):
                url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo"
                # 불필요한 공백 제거 및 대문자 변환
                search_bl = bl_no.upper().strip()
                search_mrn = mrn_no.upper().strip()
                
                params = {"crkyCn": API_KEY, "blYy": str(carg_year), "hblNo": search_bl, "cargMtNo": search_mrn}
                
                try:
                    # 타임아웃을 20초로 확장하고 재시도 로직을 위한 기초 설정 #변경내역★★
                    ...
                    if res.status_code == 200:
                        ...
                        if info is not None:
                            # 1) 상호 자동 완성 기능: 결과값을 세션에 저장 #변경내역★★
                            # HBL이 없으면 MBL을 가져오도록 우선순위 설정
                            res_bl = info.findtext("hblNo") or info.findtext("mblNo") or ""
                            res_mrn = info.findtext("cargMtNo") or ""
                            
                            st.session_state.bl_val = res_bl
                            st.session_state.mrn_val = res_mrn
                            ...
                            def get_safe_text(element, tag, length=None):
                                ...
                                if val is None: return "정보없음"
                                return val[:length] if length else val
                            ...
                            m4.metric("장치장명", get_safe_text(info, "shedNm", 10))
                            m5.metric("현재위치", get_safe_text(info, "ldpsNm", 10))
                            
                            st.divider()
                            
                            # 3) 상세 진행 내역 테이블 (헤더 고정 스크롤 적용) #변경내역★★
                            history = []
                            ...
                            if history:
                                st.write("**📑 상세 진행 단계 (최신순)**")
                                df_hist = pd.DataFrame(history)
                                # 테이블이 화면 전체를 채우도록 HTML 변환 적용 #변경내역★★
                                st.write(f'<div style="width: 100%; max-height: 400px; overflow-y: auto;">{df_hist.to_html(index=False, classes="full-width-table")}</div>', unsafe_allow_html=True)
                                
                                # 자동 완성을 즉시 반영하기 위한 리런 (조회 성공 시에만) #변경내역★★
                                st.rerun()
                            else:
                                st.info("상세 진행 내역이 존재하지 않습니다.")
                        else:
                            st.warning("⚠️ 조회 결과가 없습니다. 입항년도나 번호를 확인해 주세요.")
                except requests.exceptions.Timeout:
                    st.error("❌ 관세청 서버 응답 시간이 초과되었습니다. 잠시 후 다시 시도해 주세요.")
                except Exception as e:
                    st.error(f"❌ 통신 오류: {e}")
