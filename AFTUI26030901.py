...
# --- [AFTUI26031012] 전역 디자인 설정 (엘박스 스타일 + 10cm 고정폭) --- #변경내역★★
...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        /* [탭2 전용] 테이블 레이아웃: 화면 꽉 채우기 및 열 폭 10cm 고정 */ #변경내역★★
        .full-width-table {{ 
            width: 100% !important; 
...
# --- [Tab 2] HS정보 (전체 폭 채우기 및 협정세율 스크롤 반영) --- #변경내역★★
with tabs[1]:
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            try:
                ...
                # 2) 관세율 (rate_names와 JOIN하여 한글내역 추출) #변경내역★★
                r_q = f"""
                    SELECT r.type as '코드', 
                           n.h_name as '세율명칭', 
                           r.rate as '세율' 
                ...
                # 3) 요건 (요청하신 항목명칭 '법령명, 승인기관, 서류명'으로 변경 추출) #변경내역★★
                ...
                if not m.empty:
                    ...
                    if not r_all.empty:
                        ...
                        # [핵심] 테이블 폭 10cm(378px) 고정 및 전체 화면 채우기 스타일 적용 함수 #변경내역★★
                        def styled_rate_table(df, is_scroll=False):
                            # full-width-table 클래스로 좌우 꽉 채우기 수행
                            html = df.to_html(index=False, classes='full-width-table', justify='center')
                            # colgroup 주입: 1열(코드) 378px, 2열(명칭) 가변, 3열(세율) 378px #변경내역★★
                            styled_html = html.replace('<thead>', 
                                '<colgroup>'
                                '<col class="col-fixed-10cm">'
                                '<col class="col-flexible">'
                                '<col class="col-fixed-10cm">'
                                '</colgroup><thead>')
                            
                            # 스크롤 기능 추가 (10행 기준 약 450px 높이 제한) #변경내역★★
                            if is_scroll:
                                return f'<div style="max-height: 450px; overflow-y: auto; width: 100%; border: 1px solid #F1F5F9; border-radius: 4px;">{styled_html}</div>'
                            else:
                                return f'<div style="width: 100%;">{styled_html}</div>'
                        ...
                        st.markdown("— 협정세율 (FTA) [상위 10개 표시/스크롤]") #변경내역★★
                        st.write(styled_rate_table(rf, is_scroll=True), unsafe_allow_html=True)
                    
                    st.divider()
                    st.markdown("**🛡️ 세관장확인대상 (수출입요건)**") #변경내역★★
                    ci, ce = st.columns(2)
                    # 요건 테이블 또한 화면폭을 좌우로 가득 채우도록 div로 감쌈 #변경내역★★
                    with ci: 
                        st.markdown("[수입 요건]")
                        st.write(f'<div style="width: 100%;">{req_i.to_html(index=False, classes="full-width-table")}</div>', unsafe_allow_html=True)
                    with ce: 
                        st.markdown("[수출 요건]")
                        st.write(f'<div style="width: 100%;">{req_e.to_html(index=False, classes="full-width-table")}</div>', unsafe_allow_html=True)
                else: 
