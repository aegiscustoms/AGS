...
# --- [AFTUI26031005] 전역 디자인 설정 (엘박스 스타일) --- #변경내역★★
...
if not st.session_state.logged_in:
    st.markdown("<div style='text-align:center; padding-top:100px;'><h1 style='color:#1E3A8A; font-size:42px; font-weight:800;'>AEGIS</h1><p style='color:#64748B;'>전문 관세 행정 데이터 포털</p></div>", unsafe_allow_html=True) #변경내역★★
    cl1, cl2, cl3 = st.columns([1, 1.4, 1])
    with cl2:
        with st.form("login_form"): #변경내역★★
            l_id = st.text_input("아이디")
            l_pw = st.text_input("비밀번호", type="password")
            if st.form_submit_button("로그인"): #변경내역★★
...
with tabs[1]:
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            # 요청사항: 표준품명 테이블에서 품명(base_name) 추출 #변경내역★★
            ...
            if not m.empty:
                ...
                with cl:
                    ...
                    # 원본로직: base_name이 실제 원본파일의 '품명' 컬럼값임 #변경내역★★
                    st.info(std['base_name'].values[0] if not std.empty else "등록 정보 없음") #변경내역★★
                with cr:
                    st.markdown("**기본품명**") #변경내역★★
                    st.info(f"국문: {m['name_kr'].values[0]}\n\n영문: {m['name_en'].values[0]}") #변경내역★★
                
                st.divider()
                st.markdown("**💰 관세율 정보**") #변경내역★★
                if not r_all.empty:
                    r_all['세율'] = r_all['세율'].astype(str) + "%"
                    # A, C 우선 추출 로직 유지 #변경내역★★
                ...
                ci, ce = st.columns(2)
                with ci: st.markdown("[수입 요건]"); st.dataframe(req_i, hide_index=True, use_container_width=True) #변경내역★★
                with ce: st.markdown("[수출 요건]"); st.dataframe(req_e, hide_index=True, use_container_width=True) #변경내역★★
            else: st.error("정보 없음")

# --- [Tab 3] 통계부호 (원본 필드명 100% 복구) ---
with tabs[2]:
...
# --- [Tab 4] 화물통관 (양방향 자동입력 및 현재위치 연동) ---
with tabs[3]:
    st.markdown("<div class='custom-header'>📦 화물통관 진행정보 실시간 조회</div>", unsafe_allow_html=True) #변경내역★★
    # 세션 상태 초기화 #변경내역★★
    if "bl_input_val" not in st.session_state: st.session_state.bl_input_val = ""
    if "mrn_input_val" not in st.session_state: st.session_state.mrn_input_val = ""

    col_y, col_b, col_m = st.columns([1, 2, 2])
    with col_y: carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0)
    with col_b: bl_no = st.text_input("B/L 번호", value=st.session_state.bl_input_val, key="bl_search")
    with col_m: mrn_no = st.text_input("화물관리번호", value=st.session_state.mrn_input_val, key="mrn_search")
    
    if st.button("실시간 조회", use_container_width=True):
        API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip()
        if not API_KEY: st.error("API 키 미설정"); st.stop()
        
        with st.spinner("가져오는 중..."):
            url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo"
            params = {"crkyCn": API_KEY, "blYy": str(carg_year), "hblNo": bl_no.strip().upper(), "cargMtNo": mrn_no.strip().upper()}
            try:
                ...
                if response.status_code == 200:
                    ...
                    if info is not None:
                        # 상호 자동 입력 로직 #변경내역★★
                        st.session_state.bl_input_val = info.findtext("hblNo") or info.findtext("mblNo")
                        st.session_state.mrn_input_val = info.findtext("cargMtNo")
                        ...
                        st.rerun() # 자동 입력을 반영하기 위해 리런 #변경내역★★
                    else: st.warning("결과 없음")
...
with tabs[4]:
    ...
    if "duty_rate_v5" not in st.session_state: st.session_state.duty_rate_v5 = 8.0
    if "rate_type_v5" not in st.session_state: st.session_state.rate_type_v5 = "A"

    with st.container(border=True): #변경내역★★
        st.write("**📍 1. 과세가격(CIF) 및 품목 입력**")
        col_left, col_right = st.columns(2)
        with col_left:
            item_price = st.number_input("물품가격 (외화)", min_value=0.0, step=100.0)
            freight = st.number_input("운임 (Freight)", min_value=0)
            insurance = st.number_input("보험료 (Insurance)", min_value=0)
        with col_right:
            exchange_rate = st.number_input("적용 환율", value=1350.0)
            st.write("품목분류(HSK)") #변경내역★★
            hs_c1, hs_c2 = st.columns([0.7, 0.3])
            input_hs = hs_c1.text_input("HSK 입력", label_visibility="collapsed", placeholder="10자리")
            if hs_c2.button("적용", key="apply_v5"):
                if input_hs:
                    hsk_clean = re.sub(r'[^0-9]', '', input_hs).zfill(10)
                    conn = sqlite3.connect("customs_master.db")
                    rate_df = pd.read_sql(f"SELECT type, rate FROM rates WHERE hs_code = '{hsk_clean}' AND type IN ('A', 'C')", conn)
                    conn.close()
                    if not rate_df.empty:
                        # 즉시 반영 로직 보강 #변경내역★★
                        st.session_state.duty_rate_v5 = float(str(rate_df['rate'].values[0]).replace('%',''))
                        st.session_state.rate_type_v5 = rate_df['type'].values[0]
                        st.rerun()
            
            # 관세율/부가세율 우측 하단 배치 #변경내역★★
            r_c1, r_c2 = st.columns(2)
            applied_duty = r_c1.number_input(f"관세율({st.session_state.rate_type_v5}, %)", value=st.session_state.duty_rate_v5)
            applied_vat = r_c2.number_input("부가세율 (%)", value=10.0)
    ...
    if st.button("세액 계산 실행", use_container_width=True, type="primary"):
        tax_duty = int(cif_krw * (applied_duty / 100))
        tax_vat = int((cif_krw + tax_duty) * (applied_vat / 100))
        total_tax = tax_duty + tax_vat
        st.markdown(f"<div style='font-size: 24px; font-weight: bold; color: #B91C1C; text-align: right; background-color: #FEF2F2; padding: 20px; border-radius: 8px;'>💰 예상세액: {total_tax:,.0f} 원</div>", unsafe_allow_html=True) #변경내역★★
    ...
    with tabs[-1]:
        ...
        for i, m_name in enumerate(m_list):
            with cols[i%3]:
                ...
                if up and st.button(f"반영", key=f"btn_{m_name}"): #변경내역★★
                    df = safe_read_csv(up) #변경내역★★
                    if df is not None:
                        conn = sqlite3.connect("customs_master.db")
                        try:
                            # 개별 매핑 로직 (Length mismatch 방지) #변경내역★★
                            if m_name == "HS코드(마스터)":
                                df_map = df.iloc[:, :3]; df_map.columns = ['hs_code', 'name_kr', 'name_en']
                            elif m_name == "표준품명":
                                df_map = df.iloc[:, :4]; df_map.columns = ['base_name', 'hs_code', 'std_name_kr', 'std_name_en']
                            elif m_name == "관세율":
                                df_map = df.iloc[:, :3]; df_map.columns = ['hs_code', 'type', 'rate']
                            elif m_name == "관세율구분":
                                df_map = df.iloc[:, :2]; df_map.columns = ['code', 'h_name']
                            elif "세관장확인" in m_name:
                                df_map = df.iloc[:, :4]; df_map.columns = ['hs_code', 'law', 'agency', 'document']
                            ...
                            elif m_name == "관세율구분": df_map.to_sql("rate_names", conn, if_exists='replace', index=False)
        ...
        for i, s_name in enumerate(stat_list):
            with s_cols[i%2]:
                ...
                if s_up and st.button(f"반영", key=f"sbtn_{s_name}"): #변경내역★★
