...
with tabs[3]:
    ...
    if st.button("실시간 조회 실행", use_container_width=True):
        ...
        if not API_KEY:
            ...
            with st.spinner("관세청 유니패스에서 정보를 가져오는 중..."):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if info is not None:
                            # [안전한 텍스트 추출 함수] 데이터가 None일 때 발생하는 TypeError 원천 차단 #변경내역★★
                            def get_safe_text(element, tag, length=None):
                                val = element.findtext(tag)
                                if val is None: return ""
                                return val[:length] if length else val

                            # 1) 세션 업데이트 (입력창 상호 자동 반영)
                            st.session_state.bl_val = get_safe_text(info, "hblNo") or get_safe_text(info, "mblNo")
                            st.session_state.mrn_val = get_safe_text(info, "cargMtNo")
                            
                            # 2) 5대 주요 지표 가로 정렬 출력 #변경내역★★
                            ...
                            m4.metric("장치장명", get_safe_text(info, "shedNm", 10)) #변경내역★★
                            m5.metric("현재위치", get_safe_text(info, "ldpsNm", 10)) #변경내역★★
                            
                            st.divider()
                            
                            # 3) 상세 진행 내역 테이블 생성 #변경내역★★
                            history = []
                            ...
                            st.warning("⚠️ 해당 번호로 조회된 화물 정보가 없습니다. 입항년도와 번호를 다시 확인해 주세요.")
                except Exception as e:
