...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        /* [탭2 전용] 테이블 레이아웃 보정: 화면 끝까지 강제 확장 */ #변경내역★★
        .full-width-table {{ 
            width: 100% !important; 
            min-width: 100% !important; /* 좌측 치우침 방지 핵심 */
        ...
        .full-width-table th {{ 
            ...
            border: 1px solid #E2E8F0; /* 테두리 명확화 */
        ...
        /* 10cm(약 378px) 고정 열 및 가변 열 설정 */ #변경내역★★
                ...
                if not m.empty:
                    ...
                    if not r_all.empty:
                        ...
                        def styled_rate_table(df, is_scroll=False):
                            ...
                            if is_scroll:
                                # max-height 450px는 약 10~12행 높이입니다. #변경내역★★
                                return f'<div style="max-height: 450px; overflow-y: auto; width: 100%; display: block; border: 1px solid #E2E8F0; border-radius: 4px;">{styled_html}</div>'
                            else:
                        ...
                        st.markdown("기타세율") 
                        st.write(styled_rate_table(re_etc), unsafe_allow_html=True) #변경내역★★
                        
                        st.markdown("협정세율 (FTA) [상위 10개 표시/스크롤]") 
                        st.write(styled_rate_table(rf, is_scroll=True), unsafe_allow_html=True) #변경내역★★
