...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        /* 중앙 정렬 및 열 너비 고정 테이블 (폰트 2px 확대 반영) */ #변경내역★★
        ...
        .custom-table th {{ 
            ...
            font-size: 15px !important; /* 헤더 폰트 2px 키움 */
        }}
        .custom-table td {{ text-align: center !important; padding: 10px !important; border-bottom: 1px solid #F1F5F9; font-size: {CONTENT_FONT_SIZE}; }}
        
        /* 열 너비 강제 고정 */ #변경내역★★
        .col-code {{ width: 80px !important; }} /* 코드열 최소 폭 */
        .col-rest {{ width: auto !important; }}
    </style>
...
def init_db():
    ...
    ca.execute("INSERT OR IGNORE INTO users (id, pw, is_approved, is_admin) VALUES (?, ?, 1, 1)", ("aegis01210", admin_pw))
...
# --- [Tab 2] HS정보 (요청하신 정밀 매핑 및 디자인 적용) --- #변경내역★★
with tabs[1]:
            ...
            try:
                ...
                if not m.empty:
                    ...
                    if not r_all.empty:
                        ...
                        # 테이블 너비 및 폰트 커스텀 스타일 적용 함수 #변경내역★★
                        def styled_table(df):
                            # 코드열(0), 나머지열(1,2)에 클래스 주입 #변경내역★★
                            return df.to_html(index=False, classes='custom-table', justify='center') \
                                     .replace('<thead>', '<thead><colgroup><col class="col-code"><col class="col-rest"><col class="col-rest"></colgroup>')

                        st.markdown("**기타세율**") #변경내역★★
                        st.write(styled_table(re_etc), unsafe_allow_html=True)
                        st.markdown("**협정세율 (FTA)**") #변경내역★★
                        st.write(styled_table(rf), unsafe_allow_html=True)
                    
                    st.divider()
                    st.markdown("**🛡️ 세관장확인대상 (수출입요건)**") #변경내역★★
                    ci, ce = st.columns(2)
                    with ci: st.markdown("[수입 요건]"); st.write(req_i.to_html(index=False, classes='custom-table'), unsafe_allow_html=True) #변경내역★★
                    with ce: st.markdown("[수출 요건]"); st.write(req_e.to_html(index=False, classes='custom-table'), unsafe_allow_html=True) #변경내역★★
    ...
    with tabs[-1]:
            ...
            with cols[i%3]:
                ...
                if up and st.button(f"반영", key=f"btn_{m_name}"): #변경내역★★
                    ...
                    if df is not None:
                        conn = sqlite3.connect("customs_master.db")
                        try:
                            # [핵심] 첨부해주신 원본 파일의 컬럼 위치(Index)를 기반으로 정밀 매핑 #변경내역★★
                            if m_name == "HS코드(마스터)":
                                # 원본: 0번(HS부호), 3번(한글품목명), 4번(영문품목명) #변경내역★★
                                df_map = df.iloc[:, [0, 3, 4]]
                                df_map.columns = ['hs_code', 'name_kr', 'name_en']
                            elif m_name == "표준품명":
                                # 원본: 2번(HS부호), 1번(품명-base_name), 4번(표준_한글), 5번(표준_영문) #변경내역★★
                                df_map = df.iloc[:, [2, 1, 4, 5]]
                                df_map.columns = ['hs_code', 'base_name', 'std_name_kr', 'std_name_en']
                            elif m_name == "관세율":
                                df_map = df.iloc[:, [0, 1, 2]]; df_map.columns = ['hs_code', 'type', 'rate']
                            elif m_name == "관세율구분":
                                df_map = df.iloc[:, [0, 1]]; df_map.columns = ['code', 'h_name']
                            elif "세관장확인" in m_name:
                                df_map = df.iloc[:, [0, 1, 2, 3]]; df_map.columns = ['hs_code', 'law', 'agency', 'document']
                            ...
                            if m_name in t_map: df_map.to_sql(t_map[m_name], conn, if_exists='replace', index=False)
                            elif m_name == "관세율구분": df_map.to_sql("rate_names", conn, if_exists='replace', index=False)
                            st.success(f"{m_name} 반영 완료"); conn.close()
                        except Exception as e: st.error(f"오류: {e}")
        ...
        for i, s_name in enumerate(s_list):
            with s_cols[i%2]:
                ...
                if s_up and st.button("반영", key=f"sbtn_{s_name}"): #변경내역★★
                    ...
                    if sdf is not None:
                        ...
                        if s_name == "간이세율(2026)": sdf = sdf.iloc[:, :3]; sdf.columns = ['gani_hs', 'gani_name', 'rate']
                        elif s_name == "관세감면부호(2026)": sdf = sdf.iloc[:, :6]; sdf.columns = ['code', 'content', 'rate', 'after_target', 'installment_months', 'installment_count']
                        elif s_name == "내국세면세부호(2026)": sdf = sdf.iloc[:, :3]; sdf.columns = ['name', 'type_name', 'code']
                        elif s_name == "내국세율(2026)": sdf = sdf.iloc[:, :8]; sdf.columns = ['item_name', 'tax_rate', 'type_code', 'type_name', 'tax_kind_code', 'unit', 'tax_base_price', 'agri_tax_yn']
